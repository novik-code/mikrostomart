...
> **Last Updated:** 2026-06-01 #3 (**🖼️ SEO follow-up — Image sitemap + VideoObject + sekcje DE/EN geo-landingów**, commit `c31c0af`. **Image sitemap**: natywne pole `images` (Next 14.2+) w `sitemap.ts` — metamorfozy (16×2=32) + news (`image`) + KB (`image_url`) = **804 `<image:loc>`** + `xmlns:image`. `METAMORPHOSES` wyciągnięte z galerii do `src/data/metamorphoses.ts` (single source, import w galerii+sitemap). **VideoObject**: nowy `src/lib/youtubeSchema.ts` — server-side fetch realnych metadanych z YouTube API (`uploadDate`=publishedAt, WYMAGANE przez Google), cache 24h, **w pełni graceful** (brak schematu bez API key/demo/błąd → nigdy nie zaszkodzi GSC); 6 VideoObject na homepage z realnym uploadDate. **Sekcje „dla pacjentów zagranicznych"**: DE na `/zahnarzt-opole` (Anreise z DACH/Kostenerstattung EU 2011/24/Behandlung auf Deutsch) + EN na `/dentist-opole` (flights Wrocław/Katowice/insurance reimbursement/English) — inline locale-content (bez round-tripu pages.json). Build clean (213 stron), zweryfikowane w preview (sitemap 804 obrazów, VideoObject uploadDate real, sekcje DE/EN, /metamorfozy 200 po refaktorze galerii, 0 błędów). **Brak migracji/env var** (YOUTUBE_API_KEY/CHANNEL_ID już istnieją). **🎯 SEO Pakiet A+B+C + follow-up COMPLETE** — wszystkie priorytety audytu + odłożone P1/P2 zamknięte. <br><br>**Poprzednia: 2026-06-01 #2 (**🌍 SEO Pakiet C — dedykowane geo-landingi `/de/zahnarzt-opole` + `/en/dentist-opole`**, commit `0543d6f`. Strony indeksowane TYLKO w target locale (DE/EN), reszta noindex; Service (areaServed DACH dla zahnarzt / EU+UK+IE dla dentist) + FAQPage + BreadcrumbList schema; nowe i18n namespaces zahnarztOpole+dentistOpole (kopia dentystaOpoleCentrum + DE/EN hero override, insercja string = 0 usunięć); seoTranslations keyword-rich DE/EN; sitemap intlGeoRoutes + hreflang; audit-hreflang LOCALE_ONLY_PATHS + gałąź walidacji → **176/176 OK**. Zweryfikowane w preview (DE/EN indexable + canonical self + hreflang ×5, PL/UA noindex, areaServed DACH/EU, h1 locale-aware, 0 błędów). Build clean. Brak migracji/env var. **🎯 SEO Pakiet A+B+C COMPLETE.** Odłożone (P1/P2): Image sitemap, VideoObject YouTube, ItemList /nowosielski. <br><br>**Poprzednia: 2026-06-01 (**🔎 SEO Pakiet A+B — H1 geo + alt-texty + MedicalWebPage/Physician @id + ItemList + lazy mapa geo + hasMap place_id**, commity `2b564a9` (A) + `9db89ea` (B)). **Pakiet A**: jeden keyword-rich `<h1>` w SSR-only bloku HeroSlideshow (`heroSlideshow.seoH1`, locale-aware "Stomatolog Opole"/"Zahnarzt Opole"/...) + usunięty DUPLIKAT H1 (wcześniej SSR slide-0 i widoczny carousel oba renderowały h1); widoczny carousel + 5 slidów SSR = teraz `<h2>`. Opisowe alt-texty: metamorfozy (`BeforeAfterSlider` nowe propsy `beforeAlt/afterAlt` + tytuł metamorfozy + "Mikrostomart Opole"), OfferCarousel (brand+geo), YouTubeFeed logo ("Clinic"→brand+geo). **Pakiet B**: `baza-wiedzy/[slug]` Article→**MedicalWebPage** + author/reviewedBy Physician `@id` (`#marcin-nowosielski`) + `lastReviewed` (ZERO embedded Review/aggregateRating — GSC self-serving); `nowosielski/[slug]` BlogPosting author → Physician `@id`; **ItemList** JSON-LD na `/baza-wiedzy` (158 items, locale-aware URL); `hasMap` → `/maps/place/?q=place_id:` (Knowledge Panel) z fallback mapQuery; nowy **`LazyMapEmbed`** (IntersectionObserver + aspect-ratio zero-CLS + fallback link) na 3 stronach geo (implanty-opole, leczenie-kanalowe-opole-mikroskop, dentysta-opole-centrum). Build clean (213 stron), zweryfikowane w preview (1×h1 PL+DE, MedicalWebPage @id, ItemList 158, lazy mapa ładuje się po wejściu w viewport, hasMap place_id, 0 błędów konsoli). **Brak migracji DB ani env var.** **Odłożone (niższy ROI, P1/P2 audytu)**: Image sitemap, VideoObject YouTube, ItemList `/nowosielski`. **Następnie: Pakiet C — geo landingi `/de/zahnarzt-opole` + `/en/dentist-opole`** (dedykowane slugi, Service+FAQPage schema, areaServed DACH, hreflang, audit PUBLIC_PATHS, RevealOnScroll priority). <br><br>**Poprzednia: 2026-05-29 (**📰 Nowa aktualność: "Pełna metamorfoza uśmiechu" — case-study teaser (4 locale) + migracja 159**, commit `46ed7b0`). Zajawka pełnego studium przypadku: pacjentka trafiła jako osoba towarzysząca synowi → przełamała wieloletni strach → wieloetapowa rehabilitacja (stara protetyka → przebudowa podłoża → chirurgia + implantologia → protetyka tymczasowa → ostateczna rekonstrukcja), prowadził lek. dent. Marcin Nowosielski M.Sc. **Migracja 159** idempotent INSERT do `news` (PL+EN+DE+UA title/excerpt/content + 12 tagów SEO, treść MARKDOWN pod parser strony slug — NIE HTML). Grafika 1080×1080 WebP 113 KB (hero + OG + listing thumbnail, z `~/Downloads/ewa metamorfoza.png`). Cross-linki wewnętrzne: chirurgia/implantologia/protetyka/zespół-marcin/akredytacje/metamorfozy/rezerwacja (auto locale-prefix w EN/DE/UA). Auto z page.tsx: NewsArticle schema + BreadcrumbList + hreflang ×4 + sitemap pickup (revalidate 3600s) + meta keywords z tags; slug page `revalidate=60`. Slug: `/aktualnosci/pelna-metamorfoza-usmiechu-przypadkowa-wizyta`. Build clean. **🚨 Manual Marcin: wgrać migrację 159 TYLKO NA PRODUKCJI** (`keucogopujdolzmfajjv`) — **NIE na demo** (realne zdjęcia pacjentki + branding kliniki; zgoda dotyczy mikrostomart.pl, a DB content nie jest objęty deepSanitize). Kopia: `~/Desktop/migracje_supabase/migracja_159_news_metamorfoza_usmiechu.txt`. **Next**: pełne studium przypadku w `/baza-wiedzy` (zapowiedziane "wkrótce" w treści) + opcjonalnie FB/IG social drafts z gotowego tekstu posta.<br><br>**Poprzednia: 2026-05-28 (**📚 Faza M Wave 3: 4 cluster articles PL + fix broken cross-link**). Migracje 154 (fix broken link w 149) + 155 M-P6 higiena domowa (1305 słów) + 156 M-P2 anatomia kanałów (1144) + 157 M-P2 ICON bez wiercenia (1051, slug `u-dentysty-bez-borowania` naprawia placeholder w 151) + 158 M-P7 urazy zębów pierwsza pomoc (1223, PIERWSZY M-P7 article). **Faza M cumulative Wave 1+2+3: 9 articles PL, 16 455 słów** (migracje 149-158). Audit cross-linków: 0 broken po Wave 3. Build clean. **🚨 Manual Marcin**: wgrać 5 migracji 154-158 (154 UPDATE fix, reszta INSERT) + review ~20-25 min. **Next**: Wave 4 PL (010+019+004+MERGE C 017+018+002+021 — kompletuje M-P5/M-P7), multi-locale, off-page GBP, lub M-P3/M-P4 from scratch.<br><br>**Poprzednia: 2026-05-26 #3** (**🚨 Hotfix: /zadatek nie działa — production blocker** — commit `9a18bf3`). Pacjenci nie mogli wpłacić zadatku: `/api/cart/calculate-total` rzucał "Product not available: deposit-payment" bo produkt został ukryty w admin panelu (`is_visible=false` dla UX listingu /sklep). Fix: `SYSTEM_PRODUCT_IDS` whitelist w `cartCalculator.ts` — system products (deposit-payment + przyszłe) bypass'ują is_visible filter podczas checkout. Zero ryzyka regression, 3 linie kodu. Vercel auto-deploy. **Live verify po deploy**: /zadatek + checkout → gateway BLIK/PayU/P24 zamiast błędu.<br><br>**Poprzednia: 2026-05-26 #2** (**🔬 Faza M Wave 2: 2 gold-tier case studies "Dla lekarza" hybrid style**). Migracje **152** M-P2+M-P5 case study "Mikrochirurgia endodontyczna laserowa zęba 22 z 2-letnim follow-up" (2643 słów PL, foundation art 014 recovery) + **153** M-P5 case study "Szlif bezstopniowy + lasery Er:YAG/Nd:YAG ratowanie zniszczonego zęba" (2644 słów PL, foundation art 009 recovery). **Wave 1+2 cumulative: 5 articles, 11 732 słów PL** (3 PILLARS + 1 cluster + 2 case studies). Build clean. **🚨 Manual Marcin**: wgrać **5 migracji 149-153** na OBU Supabase + cumulative review ~30-40 min. **Sugestia next session**: (D) pauza + Marcin medical review przed dalszymi waveami, lub (A) Wave 3 PL (MERGE B+015+001) / (C) multi-locale Wave 1+2.<br><br>**Poprzednia: 2026-05-26 (**🚀 FAZA M START — Wave 1 mega-sesja: 3 PL PILLARS done w 1 dzień**). M-EXIST-1 triage 21 nowosielski.pl recovery artykułów ✅ done (CLASSIFICATION.md, 7-pillar roadmap, 6 decyzji Marcina). **Wave 1**: 3 migracje 149-151 = **6445 słów PL** w 3 PILLAR-grade articles (M-P2 Endodoncja PILLAR 2422 słów + M-P6 Higienizacja PILLAR 2110 + M-P2 cluster Próchnica 1913). Build clean. **🚨 Manual Marcin**: wgrać 3 migracje 149-151 + cumulative review ~15-20 min. **Updated Faza M roadmap: 7 pillars (zamiast 5)** — M-P6 Higienizacja + M-P7 Urazowa/dziecięca jako nowe pillary. **Next session**: (A) Wave 2 PL (014+MERGE B+015), (B) Multi-locale Wave 1 DE+EN+UA, (C) Wave 2 z M-P5 focus, (D) pauza + Marcin review.<br><br>**Poprzednia: 2026-05-25 LATE+5 (**🏆 FAZA L COMPLETE: L-8 UA cluster (3 articles) — 13 articles × 3 locale matrix done**, commit `c9bd4b6`). **Cała Faza L Premium SEO COMPLETE**. L-8 UA cluster: 3 articles, ~5325 słów UA + 47 cross-links (migracje 146-148). UA-locale adaptations: PLN+UAH+EUR currency, Львів→Ополе ~150 km, WhatsApp українською 24/7, brand names międzynarodowe preserved. **Cumulative L-5+L-6+L-7+L-8: 13 articles, ~25,708 słów multilingual, ~215 cross-links** w 3 językach (DE 5 + EN 5 + UA 3). Strongest possible Google topical authority signal dla dental implants Poland. **🚨 Manual Marcin**: 12 migracji 137-148 + ~45-60 min cumulative review. **Next**: A) M-EXIST-1 triage 21 nowosielski.pl artykułów (Fazy M foundation), B) Faza M Tryb A start (M-P2 Endodoncja pillar), C) Schema expansion, D) K-MEASURE, E) GBP optimization.<br><br>**Poprzednia: 2026-05-25 LATE+4 (🎉 L-7 EN cluster COMPLETE — 5 EN articles (Pillar + 4 clusters, UK-hybrid)**, commit `c2322b7`). **Migracje 141-145 standalone EN articles**. Cumulative L-7: **10,500 słów EN + 94 cross-links**. Mirror L-6 DE cluster z UK-hybrid adaptacjami (NHS context, GBP+EUR currency, London/Manchester/Dublin flights, post-Brexit insurance BUPA/AXA/Aviva, EU 2011/24 dla EU citizens, WhatsApp English 24/7). **Cumulative DE+EN multilingual**: 10 articles, 20,383 słów, 168 cross-links cumulative — strong topical authority signal Google. **🚨 Manual Marcin**: wgrać 5 migracji 141-145 + ~25-30 min review. Faza L 11/10+ sesji (overdelivery). **Next**: L-8 UA batch (3 art, ostatnia faza L) lub pauza + medical review przed L-8.<br><br>**Poprzednia: 2026-05-25 LATE+3 (🎉 L-6 cluster COMPLETE — wszystkie 4 articles done**, commits `abf78f1` (L-6 #3) + `a3b3920` (L-6 #4)). **Faza L 10/10 sesji COMPLETE dla DE locale**. L-6 cluster cumulative: 4 articles × ~2000 słów = **7833 słów DE total + 74 cross-links**. Migracje 137-140 standalone DE articles. Topical authority signal: complete dental implant pillar w DE (Implants overview + Knochenaugmentation + Sofortbelastung + All-on-4/6 + Komplikationen interconnected). **🚨 Manual Marcin**: wgrać 4 migracje 137-140 + cumulative review ~25-30 min. **Next**: L-7 EN batch start (5 articles) lub pauza + Marcin medical review przed L-7.<br><br>**Poprzednia: 2026-05-25 LATE+2 (🇩🇪 L-6 #2: "Sofortbelastung beim Zahnimplantat" — drugi cluster article**, commit `0f6ab3a`). Migracja 138 standalone DE, **2215 słów DE** (więcej niż L-5 + L-6 #1), 19 cross-links (12 unique destinations) — **pierwszy cross-cluster link** L-6 #1 ↔ L-6 #2 w Faza M. Image `/kb-implant-structure.webp`. Content: 13 sekcji + 20 sub-sekcji z evidence-based Voraussetzungen (ISQ ≥70 Osstell, Torque ≥35 Ncm), 8 Risikofaktoren, All-on-4/6 deep dive z Maló 2011+2019 long-term studies, Marcin's protokoll (Osstell obligatorisch, Straumann BLT/Nobel Replace/Astra Tech EV, PRF, Bruxguard). Kosten: single 6-8k PLN, All-on-4 30-40k, All-on-6 45-55k. **Lesson z sesji**: KB parser NIE obsługuje markdown tables (Vergleich section przerobiona na list format). **🚨 Manual Marcin**: wgrać migracje 137 (L-6 #1) + 138 (L-6 #2) na OBU Supabase + quick review 5-10 min. Faza L 9/10 sesji done. **Next**: L-6 #3 decision — albo "All-on-4 vs All-on-6" (już szeroko pokryte w L-6 #2 deep dive) albo "Zahnimplantat-Komplikationen".<br><br>**Poprzednia: 2026-05-25 LATE+1 (🇩🇪 L-6 #1: "Knochenaugmentation in Polen" — DE article (Faza M Tryb A start)**, commit `6efc446`). Pierwszy cluster artikel po Marcin verdict A na L-5 quality. Migracja 137 standalone DE (own group_id, hreflang DE + x-default → DE), 1890 słów, 14 cross-links, image `/kb-bone-regeneration.webp`. Lessons applied (no 4800 PLN, zero embedded Reviews, idempotent, clinic-wide round-up + cross-link do live /de). **🚨 Manual Marcin**: wgrać migrację 137 + quick review 5-10 min. Faza L 8/10 sesji done. **Next**: L-6 #2 Sofortbelastung.<br><br>**Poprzednia: 2026-05-25 LATE (🎯 L-10 — extended hreflang audit + robots/canonical checks**, commit `32990c8`). Rozszerzony `scripts/audit-hreflang.mjs` z J-2 (2026-05-12) o 8 nowych paths (akredytacje × 6, zespol × 2, gwarancje, 3× L-1/L-2 PL-only geo) + `PL_ONLY_PATHS` set (8 paths) + `GLOBALLY_NOINDEX_PATHS` set (/zadatek) + walidacja meta robots noindex + canonical link consistency. Nowy npm script `audit:hreflang` (jak `audit:sitemap`). Wynik: **168/168 OK, 0 broken** (42 paths × 4 locale = 168 URLs). L-10 z Premium SEO planu zamknięty (Faza L 7/10 sesji). Brak migracji DB, brak env var, kod-only. **Faza L progress**: L-1..L-4 ✅ (geo + gwarancje), L-5 ✅ DE article (awaiting Marcin medical review), L-9 ✅ FAQ re-translate, **L-10 ✅** cross-link audit. Pozostają: L-6/L-7/L-8 content batches (po L-5 verdict). **Następna sesja**: GBP optimization, schema expansion, long-tail audit lub czekanie L-5 verdict.<br><br>**Poprzednia: 2026-05-22 (🎯 MEGA SESJA: Cross-Link nowosielski.pl + K-6 Cennik hybrid + L-1/L-2/L-3/L-4** — 6 commitów na main). Ostatni commit: `08f0227` (L-4 /gwarancje warranty hub). Faza K Premium SEO **6/8 sesji done**, Faza L **4/10 done**. K-6 zamyka Cennik z server-side SSR (8 kategorii grid + 8 FAQ + OfferCatalog schema), L-1/L-2 dodają 3 PL-only local geo pages (`/implanty-opole`, `/leczenie-kanalowe-opole-mikroskop`, `/dentysta-opole-centrum`), L-3 helper phone format + locale-aware Dentist areaServed (DE z Sachsen/Brandenburg/Berlin = ranking boost DACH), L-4 multi-locale `/gwarancje` warranty hub z Kostenerstattung step-by-step. Plus cross-link Person.sameAs Marcina → nowosielski.pl (personal brand external) + Footer external blog link. **Brak migracji DB ani env var w żadnym z 6 commitów.** Vercel auto-deployuje. **Następna sesja**: L-5 (🚨 AI-only DE article test → decision Fazy M Tryb A) lub L-9 (foreign FAQ re-translate, niskie ryzyko).<br><br>**Poprzednia: 2026-05-21 NIGHT+1 — 🚀 PERFORMANCE OPTIMIZATION SPRINT — Options A+B+C+D + CI fixes**, 6 commitów na main. Ostatni commit: `43b76d8` (Option D HeroSlideshow text-only). Po regresji wydajności PSI (mobile 37, desktop 55 — spadek z baseline Fazy E 73/83) cumulative 4 sprinty wymierzone w największe winowajce: CLS desktop 0.219 logo + composited animations (A), framer-motion tree-shake TrustStats vanilla + LazyMotion HeroSlideshow + autoplay IO pause (B), BackgroundVideo defer 8 MB MP4 via requestIdleCallback (C), HeroSlideshow text-only eliminacja CLS 0.219 main shift + 500 KB hero-slides images z initial bundle (D). Plus 2 CI fixes (cancel-in-progress=false + Node 20→24 sync z lock file). Spodziewane PSI po Vercel deploy `43b76d8`: desktop 85-92 / mobile 85-90. **Real users (CrUX field 28-day) już dobre: LCP 2.9s / INP 162ms / CLS 0.04 — wszystkie poniżej Google "needs improvement" lub Good**. **Następna sesja**: K-6 — Cennik SEO-friendly hybrid (zachowane z poprzedniego planu).<br><br>**Poprzednia: 2026-05-21 NIGHT — MEGA SESJA part 2 — Batch SEO-1 + Batch SEO-2 + K-4 strona 3 + K-5 batch 2** (4 commity na main). Commit `e6475e4` najnowszy. Cumulative dzisiaj part 2: Person schema → Physician + 7 nowych pól (`42772c9`), dedykowane `/zespol/marcin-nowosielski` + `/zespol/elzbieta-nowosielska` + PerformerCard + MedicalProcedure.performer Physician (`299951f`), K-4 strona 3 `/oferta/stomatologia-estetyczna` 280→2200+ słów (`c67ee32`), K-5 batch 2: ortodoncja + chirurgia + protetyka rozbudowy (`e6475e4`). **Wszystkie 6 service pages /oferta/* są teraz KOMPLETNIE rozbudowane** — Marcin-voice + Marcin's expertise sekcje + RWTH/LA&HA references + cross-linking + FAQ 9 pytań + PerformerCard + MedicalProcedure.performer Physician. **Faza K Premium SEO Plan w ~80% completion** (K-1..K-5 done, K-6 cennik + K-7/K-8 case studies pozostają). **Następna sesja: K-6 — Cennik SEO-friendly hybrid (AI chat primary + SSR content dla Googlebota)**. Marcin chce ZACHOWAĆ AI chat jako primary tool (zatwierdzona filozofia premium D1=B), ale dodać SSR-rendered SEO content (kategorie usług z widełkami "od X zł", FAQ cenowe, Service+OfferCatalog schema) żeby Googlebot widział pełen kontekst cennika. **Manual taski Marcina pending krytyczne**: migracja 132 (S10-1 RLS lockdown) wciąż NIE wgrana na Supabase + smoke tests.<br><br>**Poprzednia: 2026-05-21 EVENING — MEGA SESJA part 1** (8 commitów: K-3 Person schema base + 4 sekcje /o-nas + K-4 strona 1+2 + Training Gallery + UX fixes). Commit `a1e4a1e` (docs).<br><br>**Poprzednia archived: 2026-05-21 EVENING** (`6c00fc5` najnowszy). Cumulative work dzisiaj:<br><br>**1. K-3 Person schema + 4 sekcje /o-nas** (`2ca5a2f`, docs `1e80e91`): Person schema Marcina 10 → 15 pól (alumniOf RWTH+UMW, award M.Sc. + drugi/najmłodszy w PL, memberOf PTE/ESE/PTSL/OIL, hasCredential 3×, sameAs 8 URLs). 4 nowe sekcje na /o-nas: AkredytacjeSection (grid 5 kart link do `/akredytacje/[slug]` K-2b) + CvTimeline (vertical 12 milestones 2007-2024+ static SSR) + PublicationsList (6 publikacji + 5 wystąpień) + CzelejBook (okładka 37 KB WebP + CTA). i18n × 4 locale (76 PL + GPT-4o-mini translate = 304 strings). Plus modyfikacja `scripts/translate-missing-i18n.ts` — `MESSAGES_FILE` env var override.<br><br>**2. K-3 follow-up Magazyn Stomatologiczny** (`26e3ace`): sameAs 8 → 9 URLs (dodano author profile `/a5646/Lek--dent--Marcin-Nowosielski-.html`). 4 publikacje Magazyn Stomat w PublicationsList dostały klikalny anchor "→ Zobacz" linkujący do profilu autora.<br><br>**3. UX fix — O nas pierwsza pozycja w menu** (`58d1325`): Marcin zauważył że po K-3 cały personal brand jest schowany w "Dodatki ▾" dropdown (2 kliki). Foreign markets visitor / pacjent szukający "kto leczy" typowo klika "O nas" jako pierwszy ruch. Fix: O nas → pierwsza pozycja w obu menu. Desktop top nav order: **O nas · Oferta · Cennik · Metamorfozy · Narzędzia ▾ · Dodatki ▾ · Kontakt + [Umów wizytę]**. Mobile sekcja GŁÓWNE: O nas → Umów wizytę → Oferta → Cennik → Metamorfozy → Kontakt.<br><br>**4. K-4 strona 1 /oferta/implantologia copy expansion** (`d981b39`): 280→1800 słów w stylu Marcina (style analysis z 21 artykułów nowosielski.pl recovery). Nowe sekcje: Workflow cyfrowy (6 kroków numerowanych) + Trzy poziomy złożoności (single/most/All-on-4-6) + Post-op harmonogram (Dzień 0 / Dni 1-7 / Tyg 2-12 / Mies 3-4 / Mies 6+) + Dlaczego warto u nas (Oral Surgery Academy + PTSL + M.Sc. RWTH). Plus rozbudowa benefits (6 → 8), structureText (50 → 130 słów), FAQ (4 → 9 pytań). 87 keys PL + 145 translations × 3 locale = 435 total.<br><br>**5. 🚨 K-4 HOTFIX /oferta/implantologia content invisible** (`7ee833d`): Marcin zgłosił że strona jest pusta po deploy. Root cause: `<RevealOnScroll>` opakowuje cały content. IntersectionObserver threshold:0.15 wymaga że 15% elementu (~15000px po expansion) jest widoczne — niemożliwe w viewport 800px. Element zostaje opacity:0 forever. Fix: `<RevealOnScroll priority>` (pattern z Fazy G4) — renderuje plain div bez .reveal class, bez opacity animation. Verified mainOpacity=1, content widoczny od razu w SSR. **Lesson dla K-4 strona 2-3**: każda rozbudowana service page MUSI mieć `priority`.<br><br>**6. K-4 strona 2 /oferta/leczenie-kanalowe copy expansion** (`53aa32d`): 280→2000 słów w stylu peer-reviewed Marcina (style analysis z 3 nowych artykułów Magazyn Stomat dostarczonych 2026-05-21 — art4 powtórne, art5 resorpcja, art4 poprawione). Endodoncja to dyscyplina Marcina nr 1. Nowe sekcje: Mikroskop ZEISS Extaro (4 advantages rozbudowane) + Lasery PIPS/SWEEPS (mechanizm krok po kroku + Saydjari 2016 99% E. faecalis) + Workflow 7 kroków (z konkretnymi sprzętami: Reciproc R25/R40/R50, NaOCl 5,25%, EDTA 17%, MTA, AH+, ciepła gutaperka) + Re-Endo deep dive (typowy case + 70-85% skuteczność z literatury) + Authority (Curriculum Endodontyczne PTE + ESE membership + ESE Quality Guidelines + LA&HA wykłady) + "Kiedy zęba nie da się uratować" (uczciwy cross-link do implantologii — reverse direction). FAQ 4 → 9. 76 keys PL + 105 translations.<br><br>**7. K-3+ Training Gallery** (`6c00fc5`): Marcin podrzucił folder `~/Desktop/zdjecia ze szkolen/` (21 zdjęć z FB). Strategia social proof przez association: subtelne wzmianki **dr Michał Nawrocki** (pierwsza osoba w PL z M.Sc. RWTH Aachen, mentor Marcina) i **prof. Kinga Grzech-Leśniak** (Prezes PTSL, ekspert laseroterapii) jako kursanci. Wybór 6 zdjęć: (1) Marcin + dr Nawrocki z certyfikatem (wellCAMdent) — feature, (2) Marcin + prof. Grzech-Leśniak z certyfikatem (LA&HA Symposium Poland 2022) — feature, (3) Marcin keynote za mównicą Fotona/LA&HA, (4) hands-on warsztat mikroskop, (5) group action shot, (6) ukończenie szkolenia z certyfikatami Fotona. Sharp resize 1100px WebP q82 = 89-145 KB każde, total 720 KB. Nowy komponent `TrainingGallery.tsx` (215 LOC) z 6-card grid, gold border na 2 feature, hover lift + glow, aspect-ratio per orientation. Render na /o-nas między PublicationsList a CzelejBook. i18n × 4 locale (16 keys + 48 translations).<br><br>**Verification status (4 locale × każda zmiana)**: wszystko 200 OK, build clean, zero new console errors. K-4 strona 1 + 2 mają `<RevealOnScroll priority>` jako standard po hotfix. Magazyn Stomat URLs + Nawrocki/Grzech-Leśniak captions zachowane we wszystkich translacjach (AI translate preserve proper nouns).<br><br>**Następna sesja**: K-4 strona 3 /oferta/stomatologia-estetyczna copy expansion (~1h, ostatnia z batch 1) + docs update + KOMENDA section 0 finalizacja. Po K-4 batch 1 done → K-5 batch 2 (ortodoncja + chirurgia + protetyka) jako osobna sesja → K-6 cennik refactor → K-7/K-8 case studies.<br><br>**Z S10 nadal pending manual Marcin (krytyczne!)**: wgrywka migracji 132, smoke anon REST, smoke functional, sitemap audit produkcji.
