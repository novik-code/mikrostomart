...
**Domknięcie Fazy 2.** Cennik wzbogacony o All-on-X + geo H1; 3 „cienkie" strony oferty (perio/dziecięca/zachowawcza) dostały H1 z „Opole" + sekcję „nasze podejście w Opolu".

#### Commit
- `2fb8800` — feat(seo): Faza 2D (cz.4, final) — cennik +All-on-4/geo H1 + rozbudowa 3 cienkich stron

#### Co zmienione
- **Cennik:** H1 → „Cennik stomatologiczny Opole" (heroTitle/heroAccent geo ×4). Nowa kategoria **All-on-4 / All-on-6** w `cennik-categories.ts` (`catAllon4`, priceFrom 30000/priceTo 55000, badge new, href /oferta/all-on-4) → auto w OfferCatalog schema + siatce. `CENNIK_FAQ_COUNT` 8→9 + **faqQ9/faqA9** (cena All-on-4) ×4. i18n catAllon4 {title,desc,priceLabel} ×4.
- **Cienkie strony** (`periodontologia`, `stomatologia-dziecieca`, `stomatologia-zachowawcza`): H1 +„Opole" ×4 (uwaga: pl zachowawcza title kolidował z `catZachowawcza` cennika → kotwica title+subtitle) + nowa sekcja `geoTitle`/`geoText` „nasze podejście w Opolu" (page.tsx przed PerformerCard + i18n ×4) — geo + objętość treści.

#### Weryfikacja (preview prod :3001)
- cennik H1 „Cennik stomatologiczny Opole" (de „Zahnarzt-Preisliste Opole") + kategoria All-on-4 (30 000-55 000 zł) + faqQ9; 3 cienkie strony H1 z „Opole" + geoText renderuje (4 locale). 0 raw keys (geoTitle/geoText/catAllon4 w HTML = klucze w payloadzie messages client-component, NIE leak). Build clean, test 109/109, **0 błędów konsoli**, **audit:hreflang 208/208**.

#### Brak migracji / env. Deploy: produkcja + demo.

#### 🎉 FAZA 2 KOMPLETNA (2A+2B+2C+2D). Next: **Faza 3** — treść/E-E-A-T/linkowanie (blog keyword-linker, related posts, foreign-locale fallback fix, nowe klastry KB). Stałe: Marcin manual (apex→www 308, GBP „Opole"); opcjonalnie ceny licówek/metamorfozy → Offer na geo.
...
**Faza 2D (część 1)** — premium geo-landing licówek (top money keyword estetyczny). Wzorzec implanty-opole/all-on-4-opole. 2D dostarczane reviewowalnymi kawałkami (plan: możliwy split na 2 sesje).

#### Commit
- `f4abab6` — feat(seo): Faza 2D (cz.1) — premium geo /licowki-opole

#### Co powstało
- **`licowki-opole/{page,layout}.tsx`** [NEW] — PL-only indexable + foreign noindex+canonical self, hreflang scoped `['pl']`. Hero geo → 3 USP → procedura 5 kroków (link → /oferta/stomatologia-estetyczna) → Lokalizacja Opole + LazyMapEmbed → lekarz → Reviews (UI) → 6 FAQ → CTA.
- **Schema:** BreadcrumbList + Service (Cosmetic Dentistry) + MedicalProcedure (TherapeuticProcedure/Tooth, performer Physician @id) + FAQPage. **BEZ offers** (świadomie — brak ustalonej ceny licówek w danych, nie zmyślam) + BEZ Reviews (GSC-fix).
- **i18n `licowkiOpole` ×4** (~50 kluczy) + PAGE_SEO ×4 (title geo) + BREADCRUMB ×4 + sitemap localGeoRoutes + audit PUBLIC+PL_ONLY + route-mtimes + Footer + footer.seoNav.licowkiOpole ×4.

#### Weryfikacja (preview prod :3001)
- PL indexable+canonical self; EN/DE/UA noindex+canonical self; H1 geo; Service+TherapeuticProcedure+FAQPage+Breadcrumb; bez Offer/Review; hreflang scoped PL-only (pl+x-default→/licowki-opole); 0 raw keys. Build clean, test 109/109, **audit:hreflang 204/204**.

#### Brak migracji / env. Deploy: produkcja + demo.

#### Next (reszta 2D): `/metamorfoza-usmiechu-opole` (premium geo, ten wzorzec) + rozbudowa cienkich stron (periodontologia/dziecięca/zachowawcza ≥1000 słów + H1 geo + FAQ) + cennik (+All-on-4, H1 geo) + reciprocal cross-linki estetyczna/implantologia → /metamorfozy. Marcin może podać cenę licówek → dodać Offer/PriceSpec.
...
**Faza 1C — ostatnia z quick-winów; domyka całą Fazę 1.** Marcin zwrócił uwagę, by podmiana grafiki mapy bólu nie popsuła funkcji dla użytkownika — zweryfikowane na żywo (klik→modal działa, nakładka wyrównana).

#### Commit
- `e443139` — fix(seo): Faza 1C — meta length trims + news E-E-A-T byline + mapa-bólu webp

#### Co zmienione
- **Mapa bólu (P0 perf):** `public/dental-map-premium.jpg` 2.2 MB → `.webp` 57 KB (1400×1400, kwadrat zachowany). Podmiana `src` na TYM SAMYM `<img>` w `PainMapInteractive.tsx` (style nietknięte!) + `width/height={1400}` (CLS) + `fetchPriority="high"` + `decoding="async"`. JPG ZACHOWANY (`/mapa-bolu/editor` używa go do kalibracji). Bezpieczne, bo nakładka to `<svg viewBox="0 0 100 100">` absolutnie pozycjonowana — wyrównanie zależy tylko od kwadratowego aspect-ratio, nie od pikseli.
- **News E-E-A-T (YMYL):** `aktualnosci/[slug]/page.tsx` — `<ArticleByline>` pod H1 (autor Marcin `rel=author` + link do bio + „recenzja medyczna" + „Opublikowano" `<time>`) + schema `NewsArticle` `reviewedBy` (Physician @id) + `lastReviewed`.
- **Meta długości:** `seoTranslations.ts` — 18 tytułów ≤60 + 11 opisów ≤160 (TYLKO indeksowane warianty; pominięto noindex foreign single-locale). Priorytet: geo DE/EN (zahnarzt/dentist 72→≤60), zespół, periodontologia, chirurgia, implanty-opole, dentysta-centrum. Marginalne tytuły 61-64 zostawione (wytyczne pliku: 50-65).

#### Świadomie odłożone
- **CAPS w tytułach newsów (P2):** dane w tabeli `news`, nie kod. Auto-normalizacja render-time ryzykuje zniekształcenie nazw własnych (Mikrostomart, RWTH) — bezpieczniej ręczna korekta konkretnych wpisów w DB (Marcin / sesja treści).

#### Pliki
`src/app/[locale]/mapa-bolu/PainMapInteractive.tsx`, `src/app/[locale]/aktualnosci/[slug]/page.tsx`, `src/lib/seoTranslations.ts`, `public/dental-map-premium.webp` (nowy). Auto: `public/sw.js`, `src/lib/generated-route-mtimes.ts`.

#### Weryfikacja (preview prod :3001)
- Mapa: img=webp (imgComplete, natural 1400×1400, width/height 1400, fetchPriority high), nakładka SVG 35 stref `overlayAligned: true`, klik w ząb → modal diagnozy („Ząb 47", suwaki nasilenia, objawy) — screenshot. **0 błędów konsoli.**
- Byline: `rel=author` + link Marcina + „recenzja medyczna" + „Opublikowano <time>2.06.2026", schema reviewedBy+lastReviewed, brak „Invalid Date".
- Meta: audyt długości — 0 przekroczeń indeksowanych (>64). Build clean (223), test 109/109.

#### Brak migracji / env. Deploy: produkcja + demo. **FAZA 1 KOMPLETNA (1A+1B+1C).**

#### Next: **Faza 2** — premium money pages (All-on-X strona `/oferta/all-on-4` + geo `/all-on-4-opole`, metamorfozy content rebuild, geo licówki/metamorfoza, cienkie strony). Treść AI + medical review Marcina (gate przed publikacją).
...
#### Commit
- `<TBD po push>` — feat(seo): nowa strona oferty /oferta/laser (B recovery) — Fotona LightWalker, 4 locale

#### Co powstało (wzorzec Faza K)
- **`src/app/[locale]/oferta/laser/{page,layout}.tsx`** [NEW] — server-friendly client page (RevealOnScroll priority) + layout z pageMetadata + breadcrumb (`laser`) + buildServicePageSchemas. Sekcje: hero → intro → Fotona LightWalker (2 lasery: Er:YAG 2940nm SWEEPS/PIPS + Nd:YAG 1064nm, 4 karty) → 7 zastosowań (endo/bez wiertła/chirurgia bez skalpela/periodontologia/periimplantitis/biostymulacja LLLT/estetyka) → authority M.Sc. RWTH + LA&HA → 6 benefitów → cross-link do leczenia-kanalowego → koszt (note + cennik) → 7 FAQ → PerformerCard Marcin → CTA.
- **i18n: namespace `laser` × 4 locale** (63 klucze) — **autorowane ręcznie** PL+EN+DE+UA (nie AI-translate; zachowanie terminów Fotona LightWalker/Er:YAG/Nd:YAG/SWEEPS/LLLT/LA&HA/M.Sc. RWTH). Wstawione **string-level** w pages.json (lekcja Pakiet C: indent PL/EN=4, DE/UA=2, zero round-tripu JSON → bez utraty duplikatów kluczy).
- **Schema**: serviceSchemas.ts +`/oferta/laser` × 4 locale (5 FAQ + MedicalProcedure TherapeuticProcedure/Mouth) → FAQPage + MedicalProcedure + Service + BreadcrumbList na stronie.
- **SEO/integracja**: seoTranslations.ts PAGE_SEO + seo.ts BREADCRUMB_LABELS (`laser` ×4) + sitemap.ts (+4 URL) + Footer.tsx link + common.json footer.seoNav.laser ×4 + audit-hreflang PUBLIC_PATHS + generate-route-mtimes ROUTE_FILES.
- **next.config.ts**: redirect starego slug `/oferta/laserowe-leczenie-zebow` → `/oferta/laser` (był → `/oferta`).

#### Weryfikacja (build + preview)
- `npm run build` clean — **217 stron** (było 213, +4 = laser ×4 locale).
- Preview (curl :3011): 4 locale → 200, h1 locale-aware (Stomatologia laserowa/Laser Dentistry/Laserzahnheilkunde/Лазерна стоматологія), wszystkie 4 schematy obecne, **zero raw keys/MISSING_MESSAGE**, redirect 308 starego slug → /oferta/laser, markery treści (Fotona/SWEEPS/RWTH/periimplantitis).
- **`npm run audit:hreflang`: 180/180 OK, 0 broken** (było 176; +4 laser).
- sitemap zawiera /oferta/laser ×4 + alternates.

#### Brak migracji DB ani env var. Brak grafiki OG (fallback og-home.webp) — opcjonalnie do wygenerowania później.

#### Next (B ciąg dalszy)
Kolejne brakujące strony oferty: **periodontologia → stomatologia dziecięca → stomatologia zachowawcza** (ten sam wzorzec + redirecty starych slugów już istnieją w next.config). Po B → top 🟢 luki z CLASSIFICATION.md (kserostomia, overdentures, periimplantitis PL, ósemki…) do Fazy M Wave.

---

### 2026-06-02 — 📰 Nowa aktualność: "Metamorfoza uśmiechu zapracowanego biznesmena" (case-study teaser, 4 locale) + migracja 160

**Druga aktualność typu metamorfoza** (po 159). Pacjent aktywny zawodowo — zapracowany biznesmen — potrzebował leczenia szybkiego, przewidywalnego i **bez okresu bezzębia**. Zakres: ekstrakcje → implantacje natychmiastowe → sinuslift → obciążenie natychmiastowe → protetyka tymczasowa → gojenie → ostateczna rekonstrukcja (pełnokonturowy cyrkon przykręcany na implantach). Bazą: dostarczony roboczy tekst social (FB/IG) + grafika przed/po → dopieszczone, rozwinięte i zoptymalizowane SEO.

#### Commit
- `<TBD po push>` — feat(seo): aktualność "Metamorfoza uśmiechu biznesmena" — implanty natychmiastowe (PL+EN+DE+UA) + migracja 160

#### Migracja (NIE WGRANA, idempotent, TYLKO PRODUKCJA)
- **160** `160_news_metamorfoza_biznesmen_implanty.sql` — INSERT 1 row do `news` (guard `WHERE NOT EXISTS` po slug). Kopia: `~/Desktop/migracje_supabase/migracja_160_news_metamorfoza_biznesmen_implanty.txt`.

#### Treść
- **Tytuł PL**: "Metamorfoza uśmiechu zapracowanego biznesmena — implanty z natychmiastowym obciążeniem". Slug: `metamorfoza-usmiechu-implanty-natychmiastowe-opole` (ASCII, SAFE_SLUG OK, keyword-rich: metamorfoza uśmiechu + implanty natychmiastowe + Opole). Date: `2026-06-02`.
- **Format MARKDOWN** (parser slug: `## h2`, `**bold**`, `[text](/path)` auto locale-prefix, `![alt](src)`, `* listy`). Linki POZA bold (anti-pattern check: 0 nested).
- 10 sekcji H2: intro (zapracowani pacjenci bez czasu na downtime) → Konkretne oczekiwania wymagają planu → Zakres leczenia krok po kroku (8-punktowa lista) → embedded grafika → Implanty z natychmiastowym obciążeniem (USP: bez bezzębia) → Efekt to więcej niż bielszy uśmiech (7 benefitów) → Tak pracujemy w Mikrostomart → Architekt leczenia (Marcin) → To dopiero zajawka → CTA "Potrzebujesz szybkiego rozwiązania?".
- **Cross-linki wewnętrzne**: `/oferta/chirurgia`, `/oferta/implantologia` (×2), `/oferta/protetyka` (×2), `/implanty-opole` (PL-only geo, tylko w PL), `/zespol/marcin-nowosielski`, `/akredytacje`, `/metamorfozy` (×2), `/rezerwacja`. Foreign locale: zamiast `/implanty-opole` → `/oferta/implantologia` (multi-locale indexable, unika linkowania do noindex foreign geo).
- **Tłumaczenia EN + DE + UA** (title/excerpt/content) — DACH (Zahnimplantate Sofortbelastung Polen) + UK/EU EN + UA. Te same ścieżki cross-linków (auto-prefiks locale).
- **12 tagów SEO** (PL, jedna kolumna `tags`): metamorfoza uśmiechu, implanty natychmiastowe, obciążenie natychmiastowe, implanty Opole, implantologia, protetyka na implantach, cyrkon na implantach, sinuslift, zęby w jeden dzień, leczenie bez bezzębia, Marcin Nowosielski, Mikrostomart Opole.

#### Grafika
- `/images/news/metamorfoza-usmiechu-implanty-natychmiastowe-opole/main.webp` — 1080×1080 WebP **126 KB** (sharp z `~/Downloads/maba3.png`, oryginał 1254×1254 PNG 1.8 MB, fit cover). Hero + OG + listing thumbnail + embedded w treści.

#### Auto (zero zmian kodu — wszystko z istniejącego page.tsx)
- NewsArticle schema (author → **Physician @id `#marcin-nowosielski`** z #4 follow-up) + BreadcrumbList + articleSection + wordCount + keywords z tags.
- Hreflang ×4 (PL + EN + DE + uk + x-default) — emit gdy `title_{locale}` obecny (wszystkie 4 są).
- Sitemap auto-pickup (`news` table, revalidate 3600s) — +4 URLi. Slug page `revalidate=60` → live ~60s po wgraniu.
- Listing `/aktualnosci` (NewsCarousel, revalidate 600s) — auto.

#### Build
- `npm run build` clean (exit 0). Zmiana content-only (1 SQL + 1 image) — slug page/parser bez zmian. Artykuł DB-gated — pełna weryfikacja live po wgrywce.

#### 🚨 Manual Marcin
1. **Wgrać migrację 160 TYLKO NA PRODUKCJI** (`keucogopujdolzmfajjv`) — **NIE na demo** (realne zdjęcia pacjenta + branding; DB content nie objęty deepSanitize). Plik: `~/Desktop/migracje_supabase/migracja_160_news_metamorfoza_biznesmen_implanty.txt`. Idempotent.
2. Live URLs po wgraniu (~60s): `/aktualnosci/metamorfoza-usmiechu-implanty-natychmiastowe-opole` + `/en/...` + `/de/...` + `/ua/...`.
3. (Opcjonalne) GSC re-submit sitemap; Rich Results Test (NewsArticle + BreadcrumbList); FB/IG social drafty z gotowego tekstu posta.

#### Next
- Pełne studium przypadku w `/baza-wiedzy` (zapowiedziane "wkrótce" w treści) — czeka na materiały od Marcina (zdjęcia etapów, parametry kliniczne).

#### Brak env var. Brak innych migracji.

---

### 2026-06-01 #4 — 🔎 Audyt SEO follow-up: /nowosielski indexability (P1) + route-mtimes + byline E-E-A-T + meta

**Dokładny audyt SEO** (workflow multi-agent: 7 wymiarów — technical/hreflang/metadata/schema/content-E-E-A-T/performance/linking + adwersaryjna weryfikacja każdego znaleziska P0/P1 względem kodu). Werdykt: brak P0, ogólnie zdrowo; wdrożono najważniejsze poprawki. Branch `fix/seo-audit-2026-06` → ff-only na main.

#### Commit
- `b366742` — fix(seo): audit follow-up — /nowosielski indexability + route-mtimes + byline E-E-A-T + meta

#### P1 — /nowosielski crawl-orphaning (decyzja Marcina: blog ZOSTAJE, indeksować)
- **Kontekst-korekta**: nowosielski.pl to OSOBNA, niezależna domena. Blog `mikrostomart.pl/nowosielski` zostaje ze swoimi artykułami jak był → mają być zaindeksowane (NIE migracja/deindeksacja, jak sugerował stary kontekst z 2026-05-21).
- **Listing przepisany na server component**: `nowosielski/page.tsx` był `"use client"` + `useEffect` fetch → Googlebot widział pusty `{loading}`, zero linków. Teraz fetch SERWEROWY (`blog_posts` is_published + PL fallback) → przekazuje posty jako props do nowego `NowosielskiCarousel.tsx` (client island, strzałki scroll). Linki `/nowosielski/[slug]` są w SSR HTML. `revalidate = 600`.
- **sitemap.ts +blog_posts**: nowy blok `blogRoutes` mirror `kbRoutes` — per-locale URL + hreflang po `group_id` + x-default. **+264 URL blog** (sitemap **736→863**). Defensywny `isSafeSlug` + `is_published` filter.
- **ItemList JSON-LD** na listingu (domyka odłożone P3 „ItemList /nowosielski").

#### P2 — quick wins
- **route-mtimes drift fix** (regresja freshness J-1): `scripts/generate-route-mtimes.mjs` `ROUTE_FILES` nie zawierał 14 statycznych tras emitowanych przez sitemap → spadały do `buildTime = new Date()` i każdy deploy stemplował je „zmienione teraz". Dodane: akredytacje (index + 5 slugów — data-driven, mtime z `src/data/akredytacje.ts` + plików `[slug]`), zespol/marcin + zespol/elzbieta, gwarancje, implanty-opole, leczenie-kanalowe-opole-mikroskop, dentysta-opole-centrum, zahnarzt-opole, dentist-opole. **31→45 tras** z realnym git mtime.
- **meta descriptions >160 zn.** skrócone w `seoTranslations.ts` (front-load geo+kredencje): zahnarzt-opole de 238→148, dentist-opole en 235→142, zespol/marcin-nowosielski ×4 (~248-257→135-154). Telefon zostaje w schema, nie w description.
- **NewsArticle author → Physician @id**: `aktualnosci/[slug]/page.tsx` inline Person `/o-nas` → `physicianRef` (@id `#marcin-nowosielski`, url `/zespol/marcin-nowosielski`) — naprawia reciprocal Knowledge Graph link + stale URL jednym ruchem.

#### P2 — E-E-A-T (treść medyczna YMYL)
- **Nowy `src/components/ArticleByline.tsx`** (SERVER component, brak "use client" — `<a href>` z manual locale prefix, NIE next-intl Link [lekcja H3 bug]). Inline locale Record (świadomie BEZ round-tripu i18n pages.json/common.json — wzorzec z Pakietu C, unika ryzyka indent/quote). Renderuje: widoczny autor/recenzent (link do /zespol/marcin) + „Opublikowano X · Zaktualizowano Y" z `<time datetime>` (pokazuje Zaktualizowano tylko gdy dzień modyfikacji ≠ publikacja).
- Wstawiony pod H1 na `baza-wiedzy/[slug]` + `nowosielski/[slug]`.
- `nowosielski/[slug]`: dodany `reviewedBy` (physicianRef reuse) — audyt zauważył brak (był tylko author).

#### P3 — higiena
- `robots.ts`: +`/koszyk` +`/platnosc` (thin transactional pages bez organic intent).

#### Pliki
- NEW: `src/components/ArticleByline.tsx`, `src/app/[locale]/nowosielski/NowosielskiCarousel.tsx`
- MOD: `scripts/generate-route-mtimes.mjs`, `src/app/sitemap.ts`, `src/app/robots.ts`, `src/lib/seoTranslations.ts`, `src/app/[locale]/nowosielski/page.tsx` (rewrite server), `src/app/[locale]/nowosielski/[slug]/page.tsx`, `src/app/[locale]/baza-wiedzy/[slug]/page.tsx`, `src/app/[locale]/aktualnosci/[slug]/page.tsx`
- AUTO (prebuild): `src/lib/generated-route-mtimes.ts`, `public/sw.js`

#### Weryfikacja
- `npm run build` clean (213 stron).
- **`npm run audit:sitemap` (preview): 863/863 indexable, 0 failures** — wszystkie 264 nowe URL blog 200 + bez noindex.
- Preview SSR (curl): `/nowosielski` linki artykułów w HTML + ItemList; baza-wiedzy slug byline („Autor/recenzja medyczna/Opublikowano/Zaktualizowano" + link zespol/marcin) + MedicalWebPage intact; nowosielski slug byline + Physician + reviewedBy; `/de/zahnarzt-opole` meta description 148 zn.
- route-mtimes: 45 tras z realnymi datami git (sprawdzone akredytacje/zespol/geo).

#### Brak migracji DB ani env var. Vercel auto-deploy.

#### Odłożone do follow-up (niższy priorytet/ryzyko — udokumentowane, do greenlightu Marcina)
- **Hreflang scoping** single-locale geo + PL-only paths (x-default → noindex sibling). Kosmetyka GSC — Google ignoruje hreflang do noindex, brak realnego ryzyka ranking. Wymaga `onlyLocales` w `buildHreflangAlternates` (seo.ts) + `buildAlternates` (sitemap.ts) + layouty → wieloplikowe.
- **Faza Performance / Core Web Vitals**: mapa-bolu 2.2MB JPG → next-image/webp; Navbar pełny framer-motion → LazyMotion; HomeClient 5 preset pages bundlowane → next/dynamic. Osobna sesja — riskier (CLS history Navbar/hero), wymaga pomiaru PSI po stronie Marcina.
- **Drobne P3 schema**: WebSite `@id #website` (gwarancje isPartOf wskazuje na niezadeklarowany @id), `employee` back-ref Physician na Dentist, opcjonalnie Dentist `@id` → `#dentist`, news sitemap PL/x-default guard, kbRoutes `updated_at` zamiast published_date.

#### Co działa dobrze (z audytu — potwierdzone)
- Hreflang `ua`→`uk` spójne; standalone artykuły DB emitują hreflang tylko dla istniejących tłumaczeń (group_id scoping); `localePrefix: as-needed` bez wycieku `/pl/`.
- Schema: aggregateRating z autorytatywnego GBP (nie self-computed); VideoObject odporny na brakujące pola; SearchAction świadomie usunięty.
- Kanibalizacja geo vs service dobrze zarządzana; FAQ równe we wszystkich 4 locale.

---

### 2026-06-01 #3 — 🖼️ SEO follow-up: Image sitemap + VideoObject + sekcje DE/EN geo-landingów

**Domknięcie odłożonych P1/P2 z audytu** (Image sitemap, VideoObject) + wzbogacenie treści geo-landingów DE/EN (z Pakietu C). Po tym cały program SEO (audyt A+B+C + follow-up) zamknięty.

#### Commit
- `c31c0af` — feat(seo): Image sitemap + VideoObject + sekcje DE/EN dla geo-landingów

#### Image sitemap (natywne pole `images`, Next 14.2+)
- `sitemap.ts`: `multiLocaleEntries` rozszerzony o opcjonalne `images`; helper `absImg()`.
- **Metamorfozy**: `METAMORPHOSES` wyciągnięte z `MetamorphosisGallery.tsx` (client) do **`src/data/metamorphoses.ts`** (single source) — galeria importuje, sitemap też. 16×2=32 obrazy na `/metamorfozy` (multi-locale).
- **News**: `image` dodane do selectu, `images` per wpis. **KB**: `image_url` dodane, `images` per wpis.
- Wynik: **804 `<image:loc>` + `xmlns:image`** w `/sitemap.xml`. Refaktor galerii bezpieczny (`/metamorfozy` 200, 1 h1).

#### VideoObject (homepage)
- Nowy **`src/lib/youtubeSchema.ts`**: `fetchYouTubeVideosForSchema()` (server-side, YouTube Data API, `part=snippet` → `publishedAt`+`description`+`thumbnail`, cache 24h) + `buildVideoObjectSchema()`.
- **Kluczowe**: Google WYMAGA `uploadDate` dla VideoObject — bez niego Rich Results error (a projekt jest wrażliwy na GSC). Dlatego fetch realnego `publishedAt`; **w pełni graceful** (isDemoMode / brak API key / błąd → `[]` → brak schematu).
- `app/[locale]/page.tsx`: emituje array VideoObject (6 video) z realnym uploadDate. Brak coupling-ryzyka (cache + try/catch).

#### Sekcje DE/EN „dla pacjentów zagranicznych"
- `/zahnarzt-opole` (DE): Anreise z DACH (~1,5h Sachsen, loty Wrocław/Katowice) + Kostenerstattung EU 2011/24 + Behandlung auf Deutsch + Garantie/Dokumentation.
- `/dentist-opole` (EN): getting here (flights London/Manchester/Dublin) + insurance reimbursement + English-speaking + warranty/docs.
- **Inline locale-content** (Record per locale, fallback do indexed locale) — świadomie NIE przez pages.json (uniknięcie round-tripu: różne wcięcie per-locale PL/EN=4 DE/UA=2 + duplikaty kluczy w pl).

#### Weryfikacja (Claude_Preview, build clean 213 stron)
- `/sitemap.xml`: 804 `<image:loc>`, namespace OK, próbki metamorphosis+kb. Homepage: 6 VideoObject, uploadDate `2026-05-31T...` (real). `/de/zahnarzt-opole` + `/en/dentist-opole`: sekcje intl obecne. `/metamorfozy` 200. **0 błędów konsoli.**

#### Brak migracji DB ani env var (YOUTUBE_API_KEY + YOUTUBE_CHANNEL_ID już skonfigurowane). Vercel auto-deploy.

#### 🎯 SEO program COMPLETE
Audyt (6 priorytetów) → Pakiet A (H1+alt) + B (MedicalWebPage/@id+ItemList+mapa+hasMap) + C (geo DE/EN) + follow-up (Image sitemap+VideoObject+DE/EN content). Wszystko na origin/main. Opcjonalnie: Rich Results Test (VideoObject, MedicalWebPage, Service geo) + GSC re-submit sitemap.

---

### 2026-06-01 #2 — 🌍 SEO Pakiet C: dedykowane geo-landingi DE/EN (/de/zahnarzt-opole + /en/dentist-opole)

**Trzeci pakiet z audytu SEO** (po A+B). Dedykowane geo-landingi pod rynki DACH (DE) i międzynarodowy (EN) z keyword-rich slugami. Decyzja Marcina: dedykowane slugi (nie reuse), DE+EN, push po A+B.

#### Commit
- `0543d6f` — feat(seo): Pakiet C — dedykowane geo-landingi /de/zahnarzt-opole + /en/dentist-opole

#### Co powstało
- **`/zahnarzt-opole`** (DE-indexed): layout.tsx (robots index tylko `de`, Service areaServed DACH: Opole/Poland/Germany/Sachsen/Brandenburg/Berlin/Austria/Switzerland + FAQPage + BreadcrumbList) + page.tsx (struktura jak dentysta-opole-centrum, RevealOnScroll priority, LazyMapEmbed, cross-linki). Hero DE: „Zahnarzt Opole für deutschsprachige Patienten".
- **`/dentist-opole`** (EN-indexed): analogicznie, areaServed Opole/Poland/EU/UK/Ireland. Hero EN: „Dentist in Opole for international patients".
- **i18n**: nowe namespaces `zahnarztOpole` + `dentistOpole` w `pages.json` × 4 locale — kopia `dentystaOpoleCentrum` (65 kluczy) + override hero (DE dla zahnarzt, EN dla dentist). Insercja string-level (per-file indent: PL/EN=4, DE/UA=2) → **tylko dodatki, 0 usunięć** (uniknięto round-tripu który usuwał zduplikowane klucze w pl/pages.json).
- **seoTranslations.ts**: wpisy `/zahnarzt-opole` + `/dentist-opole` (title/description/keywords keyword-rich w indeksowanym locale).
- **sitemap.ts**: `intlGeoRoutes` — `${BASE_URL}/de/zahnarzt-opole` + `/en/dentist-opole` z `buildAlternates()` hreflang.
- **audit-hreflang.mjs**: nowa `LOCALE_ONLY_PATHS` Map + gałąź walidacji (indexed locale indexable + canonical self; reszta noindex) + 2 paths w PUBLIC_PATHS.

#### Weryfikacja (Claude_Preview, build clean 213 stron)
- `/de/zahnarzt-opole` → 200, robots brak (indexable), canonical self, hreflang ×5 (pl/en/de/uk/x-default), Service+FAQPage+BreadcrumbList, areaServed Germany/Sachsen/Switzerland, h1 „Zahnarzt Opole für deutschsprachige Patienten".
- `/zahnarzt-opole` (PL) → robots `noindex, follow`. `/en/dentist-opole` → indexable, areaServed EU/UK, h1 EN.
- **`npm run audit:hreflang` (localhost): 176/176 OK, 0 broken** (44 paths × 4). 0 błędów konsoli.

#### Wzorzec dla przyszłych geo-landingów
Kopia `dentysta-opole-centrum` (layout+page) + zmiana: namespace, robots target locale, areaServed, slug. Pamiętaj: dodać do sitemap `intlGeoRoutes` + `audit-hreflang` PUBLIC_PATHS + LOCALE_ONLY_PATHS.

#### Brak migracji DB ani env var. Vercel auto-deploy.

#### 🎯 SEO Pakiet A+B+C COMPLETE
Odłożone (świadomie, P1/P2 audytu): Image sitemap (`/sitemap-images.xml`), VideoObject YouTube, ItemList `/nowosielski`. Opcjonalnie po deploy: Google Rich Results Test dla `/de/zahnarzt-opole` + `/en/dentist-opole`, GSC re-submit sitemap.

---

### 2026-06-01 — 🔎 SEO Pakiet A+B (H1 geo + alt-texty + MedicalWebPage/Physician @id + ItemList + lazy mapa geo + hasMap place_id)

**Po audycie SEO (6 priorytetów z workflow multi-agent)** Marcin zdecydował wdrożenie A→B→C. Ta sesja: **Pakiet A** (quick wins) + **Pakiet B** (technical SEO). Pakiet C (geo landingi DE/EN) jako następny krok.

#### Commity
- `2b564a9` — feat(seo): Pakiet A — pojedynczy keyword-rich H1 (geo) + opisowe alt-texty (4 locale)
- `9db89ea` — feat(seo): Pakiet B — MedicalWebPage + Physician @id, ItemList, lazy mapa geo, hasMap place_id

#### Pakiet A — co się zmieniło
- **HeroSlideshow.tsx**: blok SSR-only (clip-path) renderuje teraz **jeden `<h1>`** = `tNav("seoH1")` (keyword-rich, geo, locale-aware), a wszystkie 5 slidów (SSR) + widoczny carousel = `<h2>`. Naprawiony **duplikat H1** (wcześniej `idx===0?h1` w SSR + `isPrimary?h1` w carousel = 2× h1, żaden z frazą). Usunięty nieużywany `isPrimary`.
- **messages/{pl,en,de,ua}/common.json**: nowy klucz `heroSlideshow.seoH1` (PL "Stomatolog Opole — Mikrostomart…", DE "Zahnarzt in Opole, Polen…", EN, UA).
- **BeforeAfterSlider.tsx**: nowe opcjonalne propsy `beforeAlt/afterAlt` (fallback `t('before/after')`); 3× `<img>` używają ich zamiast "Before/After/Safety Background".
- **MetamorphosisGallery.tsx**: przekazuje opisowe alt z `currentItem.title` + "Mikrostomart Opole".
- **OfferCarousel.tsx**: alt `offer.title` → `${offer.title} — Mikrostomart, stomatologia Opole`.
- **YouTubeFeed.tsx**: logo alt "Clinic" → "Mikrostomart — gabinet stomatologiczny Opole".

#### Pakiet B — co się zmieniło
- **baza-wiedzy/[slug]/page.tsx**: `@type` Article → **MedicalWebPage**; `author` i `reviewedBy` = Physician `@id` `${appUrl}/#marcin-nowosielski` (konsolidacja z personSchemas.ts → Knowledge Graph); dodany `name` + `lastReviewed`. **ZERO embedded Review/aggregateRating** (self-serving, lekcja GSC mig 135).
- **nowosielski/[slug]/page.tsx**: BlogPosting `author` Person → Physician `@id`.
- **baza-wiedzy/page.tsx** (listing, server): **ItemList** JSON-LD z `itemListSchema()` (locale-aware URL, 158 items).
- **layout.tsx**: `hasMap` → `https://www.google.com/maps/place/?q=place_id:${googlePlaceId}` (lepszy Knowledge Panel) z fallback `mapQuery` (demo bez placeId).
- **LazyMapEmbed.tsx** (NOWY): client, IntersectionObserver (rootMargin 200px) + `aspect-ratio:3/2` (zero-CLS) + fallback link do Google Maps Place. Dodany na 3 strony geo po sekcji "Lokalizacja".

#### Weryfikacja (Claude_Preview, produkcyjny build)
- PL `/` → 1×h1 "Stomatolog Opole — Mikrostomart…"; DE `/de` → 1×h1 "Zahnarzt in Opole, Polen…".
- `/baza-wiedzy` → ItemList 158 items; `/baza-wiedzy/[slug]` → MedicalWebPage z author/reviewedBy @id + lastReviewed + BreadcrumbList + 1 h1.
- `/implanty-opole` → LazyMapEmbed: wrapper aspect-ratio (zero-CLS), iframe ładuje się PO wejściu w viewport, fallback `place_id`, title locale-aware "Lokalizacja w Opolu".
- Homepage Dentist schema `hasMap` = place_id format. **0 błędów konsoli.** Build 213 stron clean.

#### Odłożone (świadomie — niższy ROI / wyższy effort, P1/P2 audytu)
- **Image sitemap** (`/sitemap-images.xml`) — P1, ~2-3h (Next.js nie wspiera natywnie images w sitemap.ts).
- **VideoObject** dla feedu YouTube — P2, ~1h (wymaga statycznej listy video).
- **ItemList na `/nowosielski`** — listing jest client component + domena migruje na nowosielski.pl (niski priorytet).

#### Brak migracji DB ani env var. Vercel auto-deploy.

#### Next: Pakiet C — geo landingi DE/EN
`/de/zahnarzt-opole` + `/en/dentist-opole` (dedykowane slugi, indexable tylko w target locale, Service+FAQPage+BreadcrumbList schema, areaServed DACH, hreflang, wpis do sitemap + `audit-hreflang.mjs` PUBLIC_PATHS, `RevealOnScroll priority` na długim contencie). Wzorzec: kopia działającej `implanty-opole`.

---

### 2026-05-29 — 📰 Nowa aktualność: "Pełna metamorfoza uśmiechu" (case-study teaser, 4 locale)

**Zajawka (teaser) pełnego studium przypadku** zapowiadanego na stronie. Pacjentka trafiła do kliniki jako osoba towarzysząca synowi, przełamała wieloletni strach przed dentystą i przeszła wieloetapową rehabilitację. To NEWS (aktualność), nie KB — pełne case study w `/baza-wiedzy` zapowiedziane "wkrótce".

#### Commit
- `46ed7b0` — feat(seo): aktualność "Pełna metamorfoza uśmiechu" — case-study teaser (PL+EN+DE+UA) + migracja 159

#### Migracja (NIE WGRANA, idempotent)
- **159** `159_news_metamorfoza_usmiechu.sql` — INSERT 1 row do `news` (guard `WHERE NOT EXISTS` po slug). Kopia: `~/Desktop/migracje_supabase/migracja_159_news_metamorfoza_usmiechu.txt`.

#### Treść
- **Tytuł PL**: "Pełna metamorfoza uśmiechu — przypadkowa wizyta, nieprzypadkowa przemiana". Slug: `pelna-metamorfoza-usmiechu-przypadkowa-wizyta` (ASCII, SAFE_SLUG OK). Date: `2026-05-29`.
- **Format MARKDOWN** (NIE HTML) — parser strony slug obsługuje `## h2`, `**bold**`, `[text](/path)` (auto locale-prefix), `![alt](src)`. **Lekcja**: NIE zagnieżdżać linków w `**bold**` (regex bolda połyka link) — linki trzymane poza bold.
- 5 sekcji: intro + Maraton nie sprint + Etapy leczenia (5-punktowa lista z cross-linkami) + embedded grafika + Architekt sukcesu (Marcin) + To dopiero zwiastun + CTA "Boisz się dentysty?".
- **Cross-linki wewnętrzne** (SEO + internal linking): `/oferta/chirurgia`, `/oferta/implantologia`, `/oferta/protetyka`, `/zespol/marcin-nowosielski`, `/akredytacje`, `/metamorfozy`, `/rezerwacja`.
- **Tłumaczenia EN + DE + UA** (title/excerpt/content) — DACH (Zahnarzt Polen) + UA. Te same ścieżki cross-linków (parser auto-prefiksuje locale).
- **12 tagów SEO** (PL, jedna kolumna `tags` współdzielona przez locale → schema keywords + meta keywords).

#### Grafika
- `/images/news/metamorfoza-przypadkowa-wizyta/main.webp` — 1080×1080 WebP **113 KB** (sharp z `~/Downloads/ewa metamorfoza.png`, oryginał 1254×1254 PNG 1.9 MB). Użyta jako hero (slug page, 800×400 cover-crop), OG card, listing thumbnail + embedded w treści (pełen poster z etapami widoczny).

#### Auto (zero zmian kodu — wszystko z istniejącego page.tsx)
- NewsArticle schema (headline/description/image/datePublished/author Marcin/publisher/articleSection/wordCount/keywords) + BreadcrumbList.
- Hreflang ×4 (PL + EN + DE + uk + x-default) — emit tylko gdy `title_{locale}` obecny (wszystkie 4 są).
- Sitemap auto-pickup (`news` table, revalidate 3600s) — +4 URLi (PL/EN/DE/UA). Slug page `revalidate=60` → live ~60s po wgraniu migracji.
- Listing `/aktualnosci` (NewsCarousel, revalidate 600s) — artykuł pojawi się w karuzeli automatycznie.

#### Build
- `npm run build` clean (exit 0). Zmiana content-only — slug page/parser bez zmian. Artykuł nie renderuje się dopóki Marcin nie wgra migracji (DB-gated) — pełna weryfikacja live po wgrywce.

#### 🚨 Manual Marcin
1. **Wgrać migrację 159 TYLKO NA PRODUKCJI** (`keucogopujdolzmfajjv`) — **NIE na demo** (`mhosfncgasjfruiohlfo`). Powód: realne zdjęcia pacjentki przed/po + branding kliniki; zgoda dotyczy mikrostomart.pl, a treść z DB (`news.content`) NIE jest objęta `deepSanitize` (debranding pokrywa tylko i18n messages, nie content z bazy). Plik: `~/Desktop/migracje_supabase/migracja_159_news_metamorfoza_usmiechu.txt`. Idempotent — bezpieczny re-run.
2. Live URLs po wgraniu (~60s): `/aktualnosci/pelna-metamorfoza-usmiechu-przypadkowa-wizyta` + `/en/...` + `/de/...` + `/ua/...`.
3. (Opcjonalne) GSC re-submit sitemap; Google Rich Results Test (NewsArticle + BreadcrumbList).
4. (Opcjonalne) FB/IG social drafts z gotowego tekstu posta — Marcin dostarczył pełny tekst FB; mogę dodać do `social_posts` jako draft (jak Wojtek) jeśli potwierdzi platform IDs.

#### Next
- Pełne studium przypadku w `/baza-wiedzy` (krok po kroku — zapowiedziane "wkrótce" w treści aktualności). Czeka na materiały od Marcina (zdjęcia etapów, szczegóły kliniczne).

#### Brak env var. Brak innych migracji.

---

### 2026-05-28 — 📚 Faza M Wave 3: 4 cluster articles PL (M-P6 + M-P2 ×2 + M-P7) + fix broken cross-link

**Wave 3 content depth** — rozbudowa pillarów Wave 1+2 o cluster sub-articles z recovery foundation. 4 nowe artykuły PL + 1 fix migracja (broken cross-link w 149). Kierunek wybrany przez Marcina (vs multi-locale / off-page / nowe pillary).

#### Commit
- `<TBD>` — feat(seo): Faza M Wave 3 — 4 cluster articles + fix broken cross-link 149

#### Audit cross-linków Wave 1+2 (przed Wave 3)

Wykryto **2 broken cross-links** w już-live migracjach:
1. **149 (endo PILLAR)** → `/baza-wiedzy/laserowa-resekcja-mikrochirurgia-endodontyczna` — nie istnieje. Faktyczny slug 152 to `mikrochirurgia-endodontyczna-laserowa-case-study-zab-22`. **Fix: migracja 154** (UPDATE REPLACE precyzyjny string, idempotent).
2. **151 (próchnica)** → `/baza-wiedzy/u-dentysty-bez-borowania` — placeholder dla ICON. **Fix: Wave 3 artykuł 157 użył dokładnie tego slug** → link działa po wgraniu.

#### Migracje (5 nowych, NIE WGRANE, idempotent)

- **154** `m_fix_149_broken_crosslink.sql` — UPDATE REPLACE fix broken link (NIE INSERT, tylko naprawa)
- **155** `m_p6_pl_codzienna_higiena_domowa_cluster.sql` — M-P6 sub
- **156** `m_p2_pl_anatomia_kanalow_cluster.sql` — M-P2 sub
- **157** `m_p2_pl_icon_bez_wiercenia_cluster.sql` — M-P2 sub (slug `u-dentysty-bez-borowania`)
- **158** `m_p7_pl_urazy_zebow_pierwsza_pomoc.sql` — M-P7 standalone (PIERWSZY M-P7 article)

Kopie .txt: `~/Desktop/migracje_supabase/migracja_{154..158}_*.txt`.

#### Wave 3 articles

| Mig | Pillar | Tytuł | Słów | Foundation | Image |
|---|---|---|---|---|---|
| 155 | M-P6 cluster | Codzienna higiena jamy ustnej w domu | 1305 | MERGE B: 003+007+020 | kb-jak-unikac-najczestszych-bledow-higieny |
| 156 | M-P2 cluster | Ile kanałów mają zęby — anatomia | 1144 | 015 | kb-czy-zab-po-leczeniu-kanalowym-jest-martwy |
| 157 | M-P2 cluster | Leczenie próchnicy bez wiercenia (ICON) | 1051 | 001 | kb-zapomnij-o-borowaniu |
| 158 | M-P7 standalone | Urazy zębów — pierwsza pomoc | 1223 | 013 | kb-usmiech-na-ratunek-pierwsza-pomoc |

**Slugi**:
- `codzienna-higiena-jamy-ustnej-w-domu`
- `ile-kanalow-maja-zeby-anatomia`
- `u-dentysty-bez-borowania` (naprawia placeholder 151)
- `urazy-zebow-pierwsza-pomoc`

**Content highlights**:
- **155** higiena domowa: szczotkowanie (metoda Bassa, sonic), nitka vs szczoteczki międzyzębowe (Curaprox/TePe), irygator Waterpik, halitoza (LZS), czyszczenie języka, wybarwiacze płytki. Elżbieta prowadząca.
- **156** anatomia kanałów: per-typ zęba (siekacze 1, premolary 1-4, trzonowce 3-6), MB2 60-95% (Stropko 1999), dlaczego nieumiejętni pomijają, Re-Endo 70-85% vs primary 90-95%, mikroskop ZEISS + CBCT wykrywalność MB2 90-95%.
- **157** ICON: infiltracja żywicy DMG, próchnica początkowa bez wiercenia, workflow 6 kroków higienistki, zalety + ograniczenia, "sukces 70% pacjent + 30% lekarz". SEO win "leczenie próchnicy bez wiercenia".
- **158** urazy: emergency protocols (wybicie zęba mleko/sól fizjol okno 1h, mleczne NIE replantować), złamania korony 3 typy, zwichnięcia, wtłoczenie, powikłania (martwica/resorpcja), LLLT + ochraniacze sportowe. SEO win "wybity ząb pierwsza pomoc", "uraz zęba Opole".

#### Cross-link ecosystem po Wave 3 (audit: 0 broken)

Wszystkie cross-links Wave 3 wskazują na istniejące slugi (149/150/151/152/155/157). M-P2 cluster (151+156+157) ↔ M-P2 PILLAR (149) ↔ M-P2 case study (152). M-P6 cluster (155) ↔ M-P6 PILLAR (150). M-P7 (158) ↔ M-P2 (149 leczenie kanałowe po martwicy pourazowej).

#### Faza M cumulative (Wave 1+2+3)

- **9 articles PL, 16 455 słów** (Wave 1: 6445 / Wave 2: 5287 / Wave 3: 4723)
- **9 migracji INSERT (149-153, 155-158) + 1 fix (154)** = migracje 149-158
- Pillary done: M-P2 (PILLAR 149 + 3 cluster: 151/156/157 + case 152), M-P6 (PILLAR 150 + cluster 155), M-P5 (case 153), M-P7 (standalone 158)

#### Faza M roadmap status (7 pillars)

| Pillar | Status |
|---|---|
| M-P1 Implants | ✅ DONE (L-5/L-6/L-7/L-8, 3 locale) |
| **M-P2 Endodoncja** ⭐ | 🟢 PILLAR 149 + 3 cluster (151/156/157) + case study 152 = najbogatszy |
| M-P3 Estetyka | ⏳ from scratch |
| M-P4 Protetyka | ⏳ 009 partial (jako 153 M-P5 case) |
| **M-P5 Laser** | 🟡 2 case studies (152/153), zostały: MERGE C (017+018), 004 frenektomia |
| **M-P6 Higienizacja** | 🟢 PILLAR 150 + cluster 155, zostało: 019 |
| **M-P7 Urazowa/dziecięca** | 🟡 standalone 158, zostały: 002 lapisowanie, 021 odruch wymiotny |

#### M-EXIST-1 recovery wykorzystanie (po Wave 3)

Wykorzystane: 016→149, 008→150, 006+012→151, 014→152, 009→153, 003+007+020→155, 015→156, 001→157, 013→158 = **12 z 21 recovery artykułów** (57%).
Pozostałe do Wave 4+: 010 (zachowawcze pod mikroskopem), 019 (zabiegi higien), 004 (frenektomia M-P5), 017+018 (bez bólu M-P5), 002 (lapisowanie M-P7), 021 (odruch wymiotny DIRECT_REUSE). SKIP: 005, 011.

#### Manual Marcin po Wave 3

1. **Wgrać 5 migracji 154-158** na OBU Supabase (idempotent, można razem). **154 jest UPDATE** (fix), reszta INSERT.
2. **Review ~20-25 min** (4 cluster articles, krótsze ~1100-1300 słów):
   - 155 higiena domowa: Elżbieta prowadząca + brand (Curaprox/Waterpik/Sonicare) OK?
   - 156 anatomia: MB2 60-95% + Re-Endo 70-85% — zgodne?
   - 157 ICON: workflow higienistki + DMG Icon — zgodne z praktyką (Elżbieta wykonuje)?
   - 158 urazy: emergency protocols (mleko/sól fizjol, mleczne nie replantować) — medycznie poprawne?
3. GSC re-submit sitemap (~792 URLi po Wave 3)

#### Next session

- **Wave 4 PL** (dokończenie recovery): 010 + 019 + 004 + MERGE C (017+018) + 002 + 021 = ~6 artykułów → kompletuje M-P5 + M-P7 + uzupełnia M-P2/M-P6
- **Multi-locale** Wave 1-3 (DE+EN+UA dla najmocniejszych pillarów)
- **Off-page**: GBP optimization + schema expansion (EventSchema LA&HA, BookSchema Czelej)
- **M-P3 Estetyka / M-P4 Protetyka** from scratch
...
### 2026-05-26 #3 — 🚨 Hotfix: /zadatek nie działa — whitelist system products w cartCalculator (production blocker)

**Marcin zgłosił** podczas Wave 2: pacjenci nie mogą wpłacić zadatku w `/zadatek` flow. Screenshot pokazuje błąd modal: **"Błąd przy obliczaniu sumy: Product not available: deposit-payment"** podczas próby checkout (koszyk wyświetla "Wpłata Zadatku 200 PLN x 1" poprawnie, ale przycisk checkout fails).

#### Commit
- `9a18bf3` — fix(payment): /zadatek nie działa — whitelist system products w cartCalculator (production blocker)

#### Diagnoza

`/api/products` zwraca produkt `deposit-payment` (nie filtruje `is_visible`) → `/zadatek` page znajduje go i renderuje koszyk → user klika checkout → `/api/cart/calculate-total` (S2-2 server-side cart total z 2026-05-12) waliduje:
...
**Live check** (`curl https://www.mikrostomart.pl/api/products`):
```json
{
  "id": "deposit-payment",
  "name": "Wpłata Zadatku",
...
**Przyczyna**: admin (lub Marcin) ukrył produkt `deposit-payment` w panelu admin (`is_visible=false`). To **poprawne UX** dla `/sklep` listingu — nie chcemy tam "Wpłata Zadatku" obok produktów retail. **Ale** `/zadatek` flow używa **tego samego produktu** jako vehicle dla checkout — i `/api/cart/calculate-total` go odrzuca, łamiąc cały flow zadatków.

#### Fix

Dodano `SYSTEM_PRODUCT_IDS` whitelist w `src/lib/cartCalculator.ts`:
...
**Zero ryzyka regression** — change minimalny (3 linie kodu + 1 stała). Zachowuje `is_visible` filter dla wszystkich innych produktów retail. `/sklep` listing nadal ukrywa `deposit-payment` (przez inną logikę w `getProducts()` / ProductGrid filter). `/zadatek` flow teraz działa.

#### Pliki
- `src/lib/cartCalculator.ts` (+15/-2) — SYSTEM_PRODUCT_IDS whitelist + komentarz wyjaśniający production fix

#### Spodziewany efekt po Vercel deploy (~2-3 min od `9a18bf3`)

- `/zadatek` flow działa end-to-end: user wpisuje kwotę → "Dodaj do koszyka" → checkout → płatność BLIK/PayU/P24 → success
- `/sklep` listing nadal **NIE pokazuje** `Wpłata Zadatku` (poprawnie ukryte)
- Future-proof: dodanie nowych system products do whitelist (np. `consultation-payment`, `online-consultation`) zajmuje 1 linię w SYSTEM_PRODUCT_IDS

#### Manual verify po deploy

Marcin może przetestować:
1. Otwórz https://www.mikrostomart.pl/zadatek
2. Wpisz dowolną kwotę ≥50 PLN
3. "Dodaj do koszyka" + "Przejdź do płatności"
4. Powinien pokazać się gateway BLIK/PayU/P24 (zamiast błędu "Product not available")
5. **Nie trzeba kończyć płatności** — wystarczy że gateway się pojawi = fix działa

#### Brak migracji DB ani env var

Tylko code patch w `src/lib/cartCalculator.ts`. Vercel auto-deploy.

---

### 2026-05-26 #2 — 🔬 Faza M Wave 2: 2 gold-tier case studies "Dla lekarza" (M-P2 mikrochirurgia + M-P5 szlif bezstopniowy)

**Wave 2 mega-sesji** — 2 gold-tier case studies w hybrid style (academic accuracy + popularyzacja terminologii, Marcin decision M-EXIST-1 #5). Foundation: nowosielski.pl recovery 014 + 009 (najdłuższe peer-reviewed artykuły).

#### Commit
- `<TBD po push>` — feat(seo): Faza M Wave 2 — 2 case studies hybrid (M-P2 mikrochirurgia + M-P5 szlif bezstopniowy)

#### Migracje (2 nowe, NIE WGRANE, idempotent)

- **152** `m_p2_m_p5_pl_mikrochirurgia_endodontyczna_case.sql`
- **153** `m_p5_pl_szlif_bezstopniowy_lasery_case.sql`

Kopie .txt: `~/Desktop/migracje_supabase/migracja_{152,153}_*.txt`.

#### Wave 2 articles

**Migracja 152 — M-P2 + M-P5 case study "Mikrochirurgia endodontyczna laserowa — case study zęba 22 z 2-letnim follow-up"** (2643 słów PL):
- Slug: `mikrochirurgia-endodontyczna-laserowa-case-study-zab-22`
- Image: `/kb-czy-zab-po-leczeniu-kanalowym-jest-martwy-i-czy-to-bezpieczne-dla-organizmu.webp`
- Foundation: nowosielski.pl recovery art **014** (2000 słów peer-reviewed) rozbudowany do 2643 z popularyzacją + 2-letnim follow-up (oryginalny 3-mies + dodany 2-letni z artykułu kontynuacja)
- Pacjentka l. 35, ząb 22 z resorpcją wewnętrzną + ekstrudowany materiał poza wierzchołek
- Jednoseansowy zabieg: Re-Endo (Er:YAG SWEEPS 10-20 mJ + Nd:YAG MSP 1,5W 15Hz) + mikrochirurgia apikalna (Er:YAG osteotomia 250-300 mJ, apicektomia 300-330 mJ chisel tip, degranulacja 80-120 mJ) + PRF + LLLT
- 2,5 godziny zabieg, brak tabletek przeciwbólowych, pełen sukces follow-up 2-letni
- 10 sekcji: Wstęp + Abstract PL+EN + Opis pacjentki + Plan + Część endo + Część chirurgiczna + PRF + LLLT + Follow-up + Marcin's expertise + Kiedy mikrochirurgia + FAQ + Następne kroki
- Cytaty: Saydjari 2016 (SWEEPS E. faecalis), Olivi 2014, Gutknecht 2018, Choukroun 2006 PRF, Setzer & Kim 2014 (85-90% skuteczność mikrochirurgii)
- Cross-links: 149 endo PILLAR, 151 próchnica, /oferta/leczenie-kanalowe, /leczenie-kanalowe-opole-mikroskop, /oferta/implantologia

**Migracja 153 — M-P5 case study "Szlif bezstopniowy + lasery Er:YAG/Nd:YAG — ratowanie mocno zniszczonego zęba"** (2644 słów PL):
- Slug: `szlif-bezstopniowy-lasery-eryag-ndyag-ratowanie-zniszczonego-zeba-case-study`
- Image: `/kb-zab-za-zab-jak-odbudowac-utracone-usmiechy-nowa-stomatologia.webp`
- Foundation: nowosielski.pl recovery art **009** (2000 słów peer-reviewed) rozbudowany do 2644 z popularyzacją
- Pacjentka l. 38, ząb 24 z głęboką próchnicą wtórną poniżej dziąsła + 5-letnia plomba po endo
- Jednoseansowy 2,5h zabieg: laserowa gingiwektomia bezkrwawa (Er:YAG MSP 1,5W) + opracowanie ubytku (Er:YAG QSP 3W) + ultradźwięki kanały + dezynfekcja Nd:YAG (MSP 1,5W 15Hz 3×20s, eliminacja >99% E. faecalis 1 mm głębokość) + wkłady standardowe + szlif bezstopniowy + hemostaza Nd:YAG (MSP 4W 15Hz) + korona tymczasowa z druku 3D (skan intraoral + CAD + drukarka NextDent C&B) + LLLT (Nd:YAG 0,5W 2×1 min, brak tabletek przeciwbólowych)
- Pacjentka chciała "wszystko w jednej sesji bez przejściowego defektu estetycznego" — kluczowy aspekt klasycznego problemu protetyki + odpowiedź laserowa
- 11 sekcji: Wstęp + Abstract PL+EN + Opis pacjentki + Plan + Główna obawa estetyczna + Workflow 7+1 kroków + Cały zabieg w liczbach + Marcin's expertise + Kiedy zastosować + FAQ + Następne kroki
- Cytaty: Gutknecht 2018, Olivi 2014, Convissar 2016 (książka referencyjna)
- Cross-links: 149 endo PILLAR, 152 mikrochirurgia, 151 próchnica, /oferta/protetyka, /oferta/implantologia

#### Cumulative Wave 1 + Wave 2 stats

- **5 articles PL**: 149/150/151 (Wave 1, 3 PILLARS + cluster) + 152/153 (Wave 2, 2 case studies)
- **11 732 słów PL cumulative** (Wave 1: 6445 + Wave 2: 5287)
- **5 migracji 149-153** wszystkie idempotent, NIE WGRANE

#### Faza M Updated Roadmap po Wave 2

| Pillar | Status |
|---|---|
| **M-P1 Implants** | ✅ DONE (L-5/L-6/L-7/L-8) |
| **M-P2 Endodoncja** ⭐ | 🟢 PILLAR done (149) + 2 cluster sub (151 próchnica + 152 mikrochirurgia case) — strong foundation |
| M-P3 Estetyka | ⏳ AI from scratch |
| M-P4 Protetyka | ⏳ 009 partial (zrobiony jako case 153, też M-P5 — można reuse w M-P4 pillar) |
| **M-P5 Laser** | 🟢 Foundation 2 case studies done (152 mikrochirurgia + 153 szlif bezstopniowy) — gotowy do pillar from 016 → 152/153 cluster |
| **🆕 M-P6 Higienizacja** | 🟢 PILLAR done (150) + zostały: MERGE B (003+007+020), 019 |
| **🆕 M-P7 Urazowa/dziecięca** | ⏳ foundation: 013, 002 (+ 021 shared) |

#### Manual Marcin po Wave 2 (cumulative z Wave 1)

1. **Wgrać 5 migracji 149-153** na OBU Supabase (wszystkie idempotent, można razem):
   - 149 endo PILLAR, 150 higiena PILLAR, 151 próchnica cluster (Wave 1)
   - 152 mikrochirurgia case, 153 szlif bezstopniowy case (Wave 2)
2. **Cumulative review ~30-40 min** (5-7 min per article × 5):
   - 152 mikrochirurgia: ząb 22 case study real (Twoja praktyka), brand statement "uratujemy zęby po nieudanych" — OK?
   - 153 szlif bezstopniowy: ząb 24 case study real (Twoja praktyka), techniczne parametry laserów + druk 3D — wszystkie zgodne?
   - Plus 3 z Wave 1
3. GSC re-submit sitemap (~787 URLi cumulative po Wave 2)
4. (Opcjonalne) Google Rich Results Test dla 5 PL URLs

#### Next session decision

Po Marcin review Wave 1+2:
- **(A) Wave 3 PL** — MERGE B higiena cluster (003+007+020) + 015 anatomia kanałów + 001 ICON. ~3 articles, ~9h AI.
- **(B) Wave 3 z M-P3/M-P4 from scratch** — estetyka i protetyka pillarów (brak recovery foundation, full AI).
- **(C) Multi-locale Wave 1+2** — replikacja 149-153 do DE+EN+UA = +15 migracji (5 × 3 locale). Strong topical authority signal.
- **(D) Pauza** — Marcin medical review przed kontynuacją (5 PL articles dla weryfikacji)

**Sugestia AI**: **(D) Pauza** — przed dalszymi waveami warto zebrać Twój verdict na pierwszych 5 articles. Jeśli quality OK → możemy przyspieszyć z (A) Wave 3 lub (C) multi-locale.

#### Brak migracji innych ani env var

Tylko 2 nowe migracje 152-153 INSERT do articles table.

---

### 2026-05-26 — 🚀 FAZA M START: Wave 1 mega-sesja (3 PL PILLARS — M-P2 endo + M-P6 higiena + M-P2 cluster próchnica)

**🎉 Faza M Premium SEO Content Engine WYSTARTOWAŁA**. Po M-EXIST-1 triage 21 nowosielski.pl recovery artykułów, Wave 1 wytworzyła **3 PILLAR-grade articles PL** w jednej sesji = **6445 słów PL** cumulative.

#### Migracje (3 nowe, wszystkie NIE WGRANE, idempotent)

- **149** `m_p2_pl_endodoncja_mikroskopowa_pillar.sql` — M-P2 PILLAR ⭐
- **150** `m_p6_pl_higienizacja_pillar.sql` — M-P6 PILLAR (NEW pillar)
- **151** `m_p2_pl_prochnica_cluster.sql` — M-P2 cluster sub-article

Kopie .txt: `~/Desktop/migracje_supabase/migracja_{149,150,151}_*.txt`.

#### Wave 1 articles

**Migracja 149 — M-P2 PILLAR "Endodoncja mikroskopowa laserowo wspomagana w Opolu — pełen przewodnik 2026"** (2422 słów PL):
- Slug: `endodoncja-mikroskopowa-laserowo-wspomagana-opole`
- Image: `/kb-leczenie-kanalowe-laserem.webp`
- Foundation: nowosielski.pl recovery art **016** (700 słów) rozbudowany do 2422 słów
- 11 sekcji: Wstęp + Czym jest endo mikro + Mikroskop ZEISS Extaro + Laser Er:YAG SWEEPS + Laser Nd:YAG + Workflow 7 kroków + Re-Endo + Marcin's expertise (M.Sc. RWTH, PTE Curriculum, ESE, 4 publikacje Magazyn Stomat, 20-lecie PTE keynote, LA&HA, Oral Surgery Academy) + Statystyki kliniki + Kiedy zęba nie da się uratować + FAQ 6 Q&A + Następne kroki
- Marcin's brand statement (z oryginału): "chciałbym przestać wkręcać implanty, lecząc kanałowo gabinetom które tego nie lubią"
- Skuteczność primary endo 90-95% (Esposito 2013, Ng 2010) vs Re-Endo 70-85% (Friedman & Mor 2004)
- Konkretne parametry: SWEEPS Er:YAG 10-20 mJ 15 Hz, Nd:YAG 1064nm MSP 1,5W 15Hz 2×20s
- Saydjari 2016: 99% E. faecalis na 1 mm w głąb kanalików zębinowych
- Cross-links: 008 higiena, 151 próchnica, /oferta/leczenie-kanalowe, /zespol/marcin-nowosielski, /akredytacje, /leczenie-kanalowe-opole-mikroskop

**Migracja 150 — M-P6 PILLAR "Regularna higienizacja stomatologiczna jako inwestycja w zdrowie uśmiechu"** (2110 słów PL):
- Slug: `regularna-higienizacja-stomatologiczna-jako-inwestycja`
- Image: `/kb-usmiech-bez-tajemnic-najczestsze-bledy-w-higienie-jamy-ustnej.webp`
- Foundation: nowosielski.pl recovery art **008** (1500 słów — najdłuższy popularyzacyjny) rozbudowany do 2110 słów
- 9 sekcji: Wstęp + Płytka + kamień + Skaling (5 metod) + Piaskowanie (Air-Flow GBT) + Fluoryzacja (lakier 22 600 ppm) + Częstotliwość + Specjalne przypadki (IZW antybiotyk, ciąża, nadwrażliwość) + Higiena domowa (Sonicare/Curaprox/Waterpik) + Higienistka Elżbieta + FAQ 6 Q&A + Następne kroki
- Elżbieta Nowosielska (higienistka, hig. stom., współzałożycielka kliniki) jako prowadząca + akredytacje (EMS Air-Flow + GBT, LA&HA)
- Cross-links: 149 endo, 151 próchnica, /zespol/elzbieta-nowosielska, /cennik, /rezerwacja
- 4500+ zabiegów higienizacyjnych wykonanych od 2016
- Polskie Towarzystwo Stomatologiczne rekomendacje IZW (amoksycylina 2 g vs klindamycyna 600 mg)

**Migracja 151 — M-P2 cluster "Próchnica zębów — od pierwszych objawów do leczenia kanałowego"** (1913 słów PL):
- Slug: `prochnica-zebow-od-pierwszych-objawow-do-leczenia-kanalowego`
- Image: `/kb-10-niespodziewanych-nawykow-ktore-niszcza-zeby.webp`
- Foundation: MERGE Group A nowosielski.pl recovery **006 + 012** (split na 2 części, 1400 słów cumulative) zmergowane + rozbudowane do 1913 słów
- 9 sekcji: Wstęp choroba zakaźna + Mechanizm trójkąt Keyesa + Czy próchnica groźna + 4 stadia + Próchnica wtórna + Leczenie próchnicy (ICON, mikroskop, leczenie kanałowe) + Profilaktyka + FAQ 6 Q&A + Następne kroki
- Polacy w europejskim rankingu próchnicy: 12 z 32 zębów u dorosłego, 80% dzieci 6-letnich, 18/32 u 60-latków
- Cross-links: **149 endo PILLAR + 150 higiena PILLAR** (wzajemne), /oferta/leczenie-kanalowe, /leczenie-kanalowe-opole-mikroskop

#### Cross-linking ecosystem Wave 1

149 ↔ 151 (endo ↔ próchnica)
150 ↔ 151 (higiena ↔ próchnica)
Wszystkie 3 → /oferta/leczenie-kanalowe, /zespol/marcin-nowosielski, /zespol/elzbieta-nowosielska, /akredytacje, /cennik, /rezerwacja, /leczenie-kanalowe-opole-mikroskop

#### Faza M Updated Roadmap — 7 pillars (zamiast 5)

Po M-EXIST-1 triage decyzjach Marcina:

| Pillar | Status | Foundation |
|---|---|---|
| **M-P1 Implants** | ✅ DONE | L-5/L-6/L-7/L-8 (13 articles, 3 locale) |
| **M-P2 Endodoncja** ⭐ | 🟢 PILLAR done | **149** (Wave 1) + cluster sub **151** (Wave 1) + zostały: 014, 001, 010, 015 |
| M-P3 Estetyka | ⏳ | AI from scratch |
| M-P4 Protetyka | ⏳ | 009 partial + from scratch |
| M-P5 Laser | ⏳ | foundation: 014, 009, MERGE C (017+018), 004 |
| **🆕 M-P6 Higienizacja** | 🟢 PILLAR done | **150** (Wave 1) + zostały: MERGE B (003+007+020), 019 |
| **🆕 M-P7 Urazowa/dziecięca** | ⏳ | 013, 002 (+ 021 shared) |

#### Manual Marcin po Wave 1

1. **Wgrać 3 migracje 149-151** na OBU Supabase (idempotent WHERE NOT EXISTS, można razem)
2. **Cumulative review ~15-20 min** (5-7 min per article):
   - 149 endo PILLAR: brand statement "chciałbym przestać wkręcać implanty" — OK czy zmienić? Liczby 1280 implantów / 2290 RCT / 6210 pacjentów — zgodne z Prodentis API?
   - 150 higiena PILLAR: Elżbieta jako prowadząca + EMS GBT + 4500 zabiegów cumulative — OK?
   - 151 próchnica: trójkąt Keyesa + 4 stadia + ICON workflow — zgodne z Twoją praktyką?
3. GSC re-submit sitemap (~785 URLi po Wave 1)
4. (Opcjonalne) Google Rich Results Test dla 3 PL URLs

#### M-EXIST-1 triage status (zamknięty 2026-05-26)

**21 recovery artykułów triaged**:
- 1 DIRECT_REUSE (021 odruch wymiotny)
- 11 UPDATE (001, 002, 004, 008→PILLAR DONE, 009, 010, 013, 014, 015, 016→PILLAR DONE, 019)
- 7 MERGE w 3 grupach (A: 006+012→DONE, B: 003+007+020, C: 017+018)
- 2 SKIP (005 "Dlaczego drogo", 011 "NFZ vs prywatne")

**6 decyzji Marcina zamkniętych**: 005 SKIP ✓, 011 SKIP ✓, M-P7 OK ✓, M-P6 OK ✓, hybrid style ✓, 016 PILLAR confirmed ✓

**Plik triage**: `~/Desktop/NOWOSIELSKI_PL_RECOVERY/CLASSIFICATION.md` (z 7-pillar roadmap + per-article table + recommended publication order)

**Oszczędność Faza M**: ~43h AI (~54% vs from-scratch estimate)

#### Next session decision points

(A) **Wave 2 PL** (~3 articles, ~9h AI): 014 case study (M-P2/M-P5) + MERGE B higiena cluster (003+007+020) + 015 anatomia kanałów (M-P2 sub)
(B) **Multi-locale Wave 1** (~9 migracji = 3 articles × DE+EN+UA): replikacja 149/150/151 w 3 językach jak L-5/L-6/L-7/L-8 pattern. Strong topical authority signal.
(C) **Wave 2 z M-P5 focus** (~3 articles): 014 gold case study + 009 szlif bezstopniowy + MERGE C "Bez bólu" (017+018) → M-P5 Laser pillar
(D) Pauza + Marcin manual review Wave 1 (PL) → potem decyzja

Sugestia AI: **(A) Wave 2 PL** — utrzymujemy momentum w PL przed multi-locale. PL recovery pokrywa 6/7 pillarów, lepiej najpierw nakreślić topical authority w PL, potem multi-locale dla najmocniejszych.

#### Brak nowych env var lub innych migracji DB

Tylko 3 nowe migracje 149-151 INSERT do articles table.

---

### 2026-05-25 LATE+5 — 🏆 FAZA L COMPLETE: L-8 UA cluster (3 articles) — 13 articles × 3 locale matrix done

**🎉🏆 CAŁA FAZA L COMPLETE** po L-8 UA batch. Pillar + cluster matrix w 3 językach (DE + EN + UA). 13 articles total. Premium SEO Plan Faza L "Local + International Depth" zamknięta.

#### Commit

- `c9bd4b6` — feat(seo): L-8 UA cluster COMPLETE — 3 UA articles (Faza L COMPLETE 🎉)

#### L-8 UA cluster — 3 articles

**Migracja 146** UA Pillar (1538 słów):
- Slug: `implanty-zubiv-v-polshchi-vartist-yakist-opole`
- Image: `/kb-implant-procedure.webp`
- Context: Українські пацієнти w Польsce post-2022, Львів→Ополе ~150 km
- Currency: PLN + UAH + EUR

**Migracja 147** UA #1 Bone Grafting (1836 słów):
- Slug: `kistkova-augmentatsiia-v-polshchi-metody-tsiny`
- Image: `/kb-bone-regeneration.webp`
- Mirror L-6 #1 / L-7 #1 z UA adaptations

**Migracja 148** UA #2 All-on-4 vs All-on-6 (1951 słów):
- Slug: `all-on-4-all-on-6-povna-restavratsiia-shchelepy`
- Image: `/kb-implant-structure.webp`
- Mirror L-6 #3 / L-7 #3 z UA adaptations + UAH conversion

**Total L-8 UA**: 5325 słów + 47 cross-links

#### UA-locale adaptations

- **Context**: Українські пацієнти w Польsce (war 2022 → many Ukrainian patients w PL, dual residence), Львів→Ополе ~150 km via Krakowska
- **Currency**: PLN primary + UAH (Українська гривня) + EUR (zamiast GBP+EUR jak L-7 EN UK-hybrid)
- **WhatsApp українською 24/7**: "українські повідомлення обробляє наша рецепція" — confirmed earlier z L-9 FAQ DE/UA re-translate (Marcin's acceptance pattern)
- **Distances**: Львів (3.5h car / 8-10h train via Перемишль), Київ (12-14h car or flight Wrocław/Kraków)
- **Direct languages**: Marcin/Elżbieta speak Polish, English, German. Ukrainian handled by reception staff via WhatsApp 24/7
- **Brand names**: international preserved (Geistlich Bio-Oss/Bio-Gide, Straumann BLT, Nobel Replace, Astra Tech EV, Osstell Beacon, ZEISS Extaro)
- **Marcin akredytacje preserved** (universal credibility): M.Sc. RWTH Ахен 2021 + LA&HA + PTE/ESE/PTSL

#### 🏆 FAZA L CUMULATIVE — wszystkie articles przez wszystkie sesje

| Sesja | Status | Articles | Słów |
|---|---|---|---|
| L-1..L-4 | ✅ | 4 stron (geo + gwarancje + akredytacje + zespol) | — |
| L-5 DE pillar | ✅ verdict A | 1 | 2050 |
| L-9 FAQ re-translate | ✅ | DE+UA FAQs | — |
| L-10 hreflang audit | ✅ | script + 168/168 OK | — |
| L-6 cluster DE | ✅ | 4 (Knochenaugm + Sofortbel + AllOn4/6 + Komplik) | 7833 |
| L-7 cluster EN | ✅ | 5 (Pillar + 4 clusters, UK-hybrid) | 10,500 |
| **L-8 cluster UA** | **✅** | **3 (Pillar + Bone + AllOn4/6)** | **5325** |
| **TOTAL FAZA L** | **✅ COMPLETE** | **13 articles + geo pages** | **~25,708 słów** |

#### Cumulative content multilingual stats

- **DE**: 5 articles (L-5 pillar + L-6 cluster) ~9883 słów + ~74 cross-links
- **EN**: 5 articles (L-7 cluster) ~10,500 słów + ~94 cross-links
- **UA**: 3 articles (L-8 cluster) ~5325 słów + ~47 cross-links
- **Total**: **13 articles, ~25,708 słów multilingual, ~215 cross-links cumulative**
- **3-language pillar+cluster matrix**: complete topical authority signal dla dental implants Poland

#### Topical authority signal Google — wszystkie 3 locale

Complete dental implant pillar w DE + EN + UA z 13 interconnected articles + 5 base landing pages (akredytacje, zespol/marcin, gwarancje, oferta/implantologia, dla-pacjentow-przyjezdnych) = **strongest possible Google signal** dla:
- DACH market (Zahnimplantat Polen, German-speaking dental tourism)
- UK/Ireland/EU English-speaking (post-Brexit dental tourism)
- UA market (українські пацієнти, war refugees w PL, cross-border)

#### Lessons applied (cumulative L-5 + L-6 + L-7 + L-8)

| Lesson | Status |
|---|---|
| No fake 4800 PLN breakdown (mig 134) | ✅ all 13 articles |
| Zero embedded Reviews schema (GSC mig 135) | ✅ all 13 articles |
| KB parser-compatible: lists, NO markdown tables | ✅ all 13 articles |
| Clinic stats round-up + cross-link do /{locale} live | ✅ all 13 articles |
| Standalone language hreflang (own group_id) | ✅ all 13 articles |
| Idempotent INSERT WHERE NOT EXISTS guard + kopie .txt | ✅ migracje 137-148 |
| Cross-cluster reinforcement (Pillar + clusters interconnected) | ✅ wszystkie 13 articles |
| WhatsApp українською 24/7 (L-9 acceptance pattern) | ✅ stosowany w L-8 UA |
| UK-hybrid adaptations (NHS, BUPA, GBP, London flights) | ✅ L-7 EN cluster |

#### 🚨 Manual Marcin po Faza L COMPLETE (CUMULATIVE 12 migracji)

**Można wgrać wszystkie razem — idempotent**:

1. **DE cluster L-6** (4 migracje):
   - `migracja_137_l6_de_knochenaugmentation_article.txt`
   - `migracja_138_l6_de_sofortbelastung_article.txt`
   - `migracja_139_l6_de_all_on_4_vs_all_on_6_article.txt`
   - `migracja_140_l6_de_komplikationen_article.txt`

2. **EN cluster L-7** (5 migracji):
   - `migracja_141_l7_en_dental_implants_poland_pillar.txt`
   - `migracja_142_l7_en_bone_grafting_article.txt`
   - `migracja_143_l7_en_immediate_loading_article.txt`
   - `migracja_144_l7_en_all_on_4_vs_all_on_6_article.txt`
   - `migracja_145_l7_en_complications_article.txt`

3. **UA cluster L-8** (3 migracje):
   - `migracja_146_l8_ua_dental_implants_poland_pillar.txt`
   - `migracja_147_l8_ua_bone_grafting_article.txt`
   - `migracja_148_l8_ua_all_on_4_vs_all_on_6_article.txt`

**Live URLs po wgraniu (13 nowych articles + 1 z L-5 już istniejący)**:
- L-5 DE pillar: `/de/baza-wiedzy/zahnimplantate-in-polen-kosten-qualitaet-opole` (already live)
- L-6 DE: 4 URLs (`/de/baza-wiedzy/knochenaugmentation-*`, `*-sofortbelastung-*`, `*-all-on-4-vs-all-on-6-*`, `*-zahnimplantat-komplikationen-*`)
- L-7 EN: 5 URLs (`/en/baza-wiedzy/dental-implants-in-poland-*`, `*-bone-grafting-*`, `*-immediate-loading-*`, `*-all-on-4-vs-all-on-6-*`, `*-dental-implant-complications-*`)
- L-8 UA: 3 URLs (`/ua/baza-wiedzy/implanty-zubiv-v-polshchi-*`, `*-kistkova-augmentatsiia-*`, `*-all-on-4-all-on-6-*`)

**Cumulative review ~45-60 min** total (5-7 min per article × 12 new + L-5 already reviewed):
- Wszystkie articles użyć pattern weryfikacji L-5 verdict A
- Per-locale specifics weryfikacja (UK NHS dla EN, war/Lwów context dla UA, BEMA/GOZ dla DE)
- **CRITICAL UA**: WhatsApp українською 24/7 — confirm że ten claim jest realny

**GSC re-submit sitemap** po wszystkich wgrywkach (~780+ URLi cumulative)

#### Spodziewany efekt po deploy + GSC pickup 2-4 tyg (cumulative)

- **Topical authority boost** w 3 językach: strongest possible Google signal dla international dental tourism Poland
- **Multi-market impressions**:
  - DACH: "Zahnimplantat Polen Kosten", "Knochenaugmentation Polen", "All-on-4 Polen"
  - UK/EU EN: "dental implants Poland cost", "bone grafting abroad", "All-on-4 Poland GBP"
  - UA: "імпланти зубів Польща ціна", "імпланти Польща Львів", "All-on-4 Польща"
- **Long-tail traffic**: NHS shortage alternatives, post-Brexit dental tourism, war-displaced Ukrainian dental access
- **CTR boost**: Article schema rich snippets w SERP

#### Next phase decision points (po Faza L COMPLETE)

| Opcja | Effort | Wartość |
|---|---|---|
| **A. M-EXIST-1 triage** (21 nowosielski.pl recovery articles) | ~1h Marcin + 30 min AI | Foundation dla Fazy M, ~70-80h pracy save |
| **B. Faza M Tryb A start** — M-P2 Endodoncja pillar | 1-2 sesje/tydzień | Drugi pillar, Marcin's #1 specialty |
| **C. Schema expansion** (EventSchema LA&HA + BookSchema Czelej + ScholarlyArticle Magazyn Stomat) | 2h AI | Niche E-E-A-T boost Marcin entity |
| **D. K-MEASURE** — pomiar Fazy K (6 tyg od K-6 deploy ~1 lipca) | 30 min AI + 30 min Marcin | Empirical validation premium positioning |
| **E. GBP optimization audit** (z PLAN_SEO_DALSZE) | 1h AI + 2h Marcin | Local SEO + GBP authoritative aggregateRating |

**Sugestia**: **Opcja A (M-EXIST-1 triage)** — Marcin + AI triage 21 nowosielski.pl artykułów (~1h). Output classification: DIRECT REUSE / UPDATE / MERGE / SKIP. Pozwala wykorzystać 13,642 słów istniejącego pillar-grade content jako foundation Fazy M (~70-80h pracy save). Po triage → start M-P2 Endodoncja (Marcin's #1 specialty z 4 publikacjami Magazyn Stomat).

#### Brak migracji innych ani env var

Tylko 3 migracje 146-148 INSERT do articles table.

---

### 2026-05-25 LATE+4 — 🎉 L-7 EN cluster COMPLETE — 5 EN articles (Pillar + 4 clusters, UK-hybrid)

**🏆 Cały L-7 batch EN complete** — 5 articles × ~2100 słów = **10,500 słów EN total + 94 cross-links cumulative**. Mirror L-6 DE cluster z UK-hybrid adaptacjami (NHS context, GBP+EUR currency, London/Manchester/Dublin flights, post-Brexit insurance, EU 2011/24, WhatsApp English 24/7).

#### Commit

- `c2322b7` — feat(seo): L-7 EN cluster COMPLETE — 5 articles (Pillar + 4 clusters, UK-hybrid)

#### 5 articles + 5 migracje (141-145)

| Migracja | Article | Słów | Cross-links | Image |
|---|---|---|---|---|
| **141** | EN Pillar: Dental Implants in Poland 2026 | 1911 | 16 | `/kb-implant-procedure.webp` |
| **142** | EN #1: Bone Grafting Methods, Costs | 2134 | 17 | `/kb-bone-regeneration.webp` |
| **143** | EN #2: Immediate Loading Requirements | 2401 | 20 | `/kb-implant-structure.webp` |
| **144** | EN #3: All-on-4 vs All-on-6 | 2010 | 21 | `/kb-implant-procedure.webp` |
| **145** | EN #4: Complications Prevention | 2044 | 20 | `/kb-implant-structure.webp` |

#### UK-hybrid adaptations vs DE

- **Currency**: GBP primary + EUR + PLN (zamiast tylko EUR+PLN)
- **NHS context**: UK dental crisis kontekst dla pricing comparison (NHS waiting times → private gap)
- **Flight times**: London Stansted/Luton (~2h), Manchester (2.5h), Dublin (2.5h), Edinburgh seasonal → Wrocław Airport (1h drive)
- **Post-Brexit insurance**: BUPA, AXA, Aviva, Vitality, Cigna explicitly mentioned dla UK patients (UK NIE objęte EU 2011/24 po Brexicie)
- **EU 2011/24 Cross-Border Healthcare Directive**: dla EU citizens (Ireland, Germany, Austria, France, Sweden)
- **WhatsApp English 24/7**: "Marcin and Elżbieta speak fluent English"
- **Brand names**: international preserved (Geistlich Bio-Oss/Bio-Gide, Straumann BLT, Nobel Replace CC, Astra Tech EV, Osstell Beacon, ZEISS Extaro Microscope)
- **No BEMA/GOZ**: replaced z generic UK private insurance + EU healthcare directive context

#### L-7 cluster cumulative stats

- **5 articles × ~2100 słów = 10,500 słów EN total** (~50% MORE than L-6 DE: 7833 słów)
- **94 cross-links cumulative** (cross-cluster reinforcement: EN pillar + inter-EN clusters + EN landing pages + /en homepage live + /implanty-opole PL)
- **5 standalone EN articles**: każdy own gen_random_uuid() group_id, hreflang tylko EN + x-default → EN

#### Lessons applied (cumulative L-5 + L-6 + L-7)

| Lesson | Status |
|---|---|
| No fake 4800 PLN breakdown (mig 134) | ✅ all 5 EN articles |
| Zero embedded Reviews schema (GSC mig 135) | ✅ all 5 EN articles |
| KB parser-compatible: lists, NO markdown tables (L-6 #2 lesson) | ✅ Vergleich sections as lists |
| Clinic stats round-up + cross-link do /en homepage live | ✅ all 5 EN articles |
| Standalone EN hreflang (own group_id) | ✅ all 5 EN articles |
| Idempotent INSERT WHERE NOT EXISTS guard + kopie .txt | ✅ migracje 141-145 |
| Cross-cluster reinforcement (Pillar + 4 clusters interconnected) | ✅ wszystkie 5 articles cross-link |

#### Faza L progress po L-7 cluster

- L-1..L-4 ✅
- L-5 ✅ DE pillar (verdict A)
- L-9 ✅ FAQ, L-10 ✅ hreflang audit
- L-6 cluster ✅ COMPLETE (4 DE articles, 7833 słów)
- **L-7 EN cluster ✅ COMPLETE** (5 EN articles, 10,500 słów) — TEN sprint
- ⏳ L-8 UA batch (3 art) — ostatnia faza L pozostała

#### Cumulative content multilingual stats (L-5 + L-6 + L-7)

- **DE content**: 5 articles (L-5 pillar + 4 L-6 clusters) = ~9883 słów
- **EN content**: 5 articles (L-7 pillar + 4 clusters) = ~10,500 słów
- **Total**: 10 articles dental implants topical authority, **20,383 słów multilingual** w 2 językach + **168 cross-links cumulative**

#### Topical authority signal Google (cumulative DE + EN)

- **Polski dental implants pillar w 2 językach** — strong international SEO signal
- DE: Zahnimplantate + Knochenaugmentation + Sofortbelastung + All-on-4/6 + Komplikationen interconnected
- EN: Dental Implants + Bone Grafting + Immediate Loading + All-on-4 vs All-on-6 + Complications interconnected
- Cross-language reinforcement (każdy artykuł cross-linkuje do /en lub /de pillar przez homepage `/en` lub `/de`)

#### 🚨 Manual Marcin po deploy

1. **Wgrać 5 migracji 141-145** na OBU Supabase (idempotent, można razem):
   - `~/Desktop/migracje_supabase/migracja_141_l7_en_dental_implants_poland_pillar.txt`
   - `~/Desktop/migracje_supabase/migracja_142_l7_en_bone_grafting_article.txt`
   - `~/Desktop/migracje_supabase/migracja_143_l7_en_immediate_loading_article.txt`
   - `~/Desktop/migracje_supabase/migracja_144_l7_en_all_on_4_vs_all_on_6_article.txt`
   - `~/Desktop/migracje_supabase/migracja_145_l7_en_complications_article.txt`

2. **Live URLs** po wgraniu (5 articles):
   - `/en/baza-wiedzy/dental-implants-in-poland-costs-quality-opole`
   - `/en/baza-wiedzy/bone-grafting-poland-methods-costs-success-rates`
   - `/en/baza-wiedzy/immediate-loading-dental-implants-requirements-risks`
   - `/en/baza-wiedzy/all-on-4-vs-all-on-6-comparison-costs-success-rates`
   - `/en/baza-wiedzy/dental-implant-complications-prevention-treatment`

3. **Cumulative review ~25-30 min** (5-7 min per article):
   - Fluency EN per article (Marcin + Elżbieta speak EN — confirm że UK-style English vocabulary OK, lub adjustments needed)
   - UK insurance references (BUPA, AXA, Aviva, Vitality, Cigna) zgodne z reality
   - GBP pricing approximations (~£6,000-8,000 for All-on-4) — verify currency conversion accurate
   - WhatsApp English 24/7 — confirm operational capacity dla EN patients
   - Premium brands wciąż używane (Geistlich Bio-Oss/Bio-Gide, Straumann BLT, Nobel Replace, Astra Tech EV, Osstell Beacon)?

4. **GSC re-submit sitemap** po wgrywce (~775+ URLi po L-7 — auto-pickup z articles table)

5. (Opcjonalne) Google Rich Results Test dla 5 EN URLs: Article + BreadcrumbList eligibility

#### Spodziewany efekt po deploy + GSC pickup 2-4 tyg

- **Topical authority boost** w EN: pillar + 4 clusters interconnected = strong Google signal dla UK/Ireland/EU English-speaking dental tourism
- **Impressions na EN queries**:
  - "dental implants Poland cost" / "dental implants Poland UK patients"
  - "bone grafting abroad" / "bone grafting Poland UK"
  - "All-on-4 Poland cost GBP" / "All-on-4 abroad UK"
  - "immediate loading dental implant"
  - "peri-implantitis treatment" / "dental implant complications"
  - "dental tourism Poland UK"
- **Long-tail UK queries**: NHS dental shortage alternatives, private insurance dental abroad, Brexit dental treatment EU
- **CTR boost**: Article schema rich snippets w SERP UK/Ireland/EU

#### Next session decision

**Po L-7 EN cluster** (Marcin verdict EN quality required przed L-8 UA):
- **(A) L-8 UA batch** (3 articles) — kompletuje całą Fazę L. Tematy: Implanty zubiv Polshcha, Augmentatsiya kistki, Imediat navantazhennya. UA market mniejszy, niższy priorytet ale completing the matrix.
- **(B) Pauza + Marcin medical review L-5 + L-6 + L-7** — ~50-60 min cumulative review przed inwestycją w UA (potwierdzenie że quality pattern działa)
- **(C) Faza M Tryb A start** — rolling content engine 60-80 articles przez 16-20 tyg, po L-8 done lub równolegle
- **(D) Schema expansion** (EventSchema LA&HA + BookSchema Czelej + ScholarlyArticle Magazyn Stomat) — niche E-E-A-T boost Marcin entity

#### Brak migracji innych ani env var

Tylko 5 migracje 141-145 INSERT do articles table.

---

### 2026-05-25 LATE+3 — 🎉 L-6 cluster COMPLETE: All-on-4 vs All-on-6 (#3) + Komplikationen (#4)

**🏆 Cały L-6 batch DE complete** — 4 articles × ~2000 słów = **7833 słów DE total + 74 cross-links cumulative**. Topical authority signal w Google: complete dental implant pillar w DE.

#### Commits

- `abf78f1` — feat(seo): L-6 #3 — DE article "All-on-4 vs All-on-6 Vergleich"
- `a3b3920` — feat(seo): L-6 #4 — DE article "Zahnimplantat-Komplikationen" (L-6 cluster COMPLETE)

#### L-6 #3 — All-on-4 vs All-on-6 Vergleich

**Migracja 139** (266 LOC):
- Slug: `all-on-4-vs-all-on-6-vergleich-kosten-erfolgsquote`
- 1836 słów DE, 21 cross-links, image `/kb-implant-procedure.webp` (reuse L-5)

**Content highlights**:
- **All-on-4 concept** (Maló 2003): 2 anterior + 2 distal anguliert 30-45° → omija Sinus/Nervkanal → oft bez augmentation
- **All-on-6 concept** (Misch protocols): 6 gerade verteilte → lepsza last-distribution, więcej Reserve, częściej augmentation
...
- **Kosten**: 30-40k / 37-47k / 45-55k PLN; Deutschland 18-30k / 25-40k EUR → Ersparnis 50-70%
- **Wissenschaftliche Evidenz**: Maló 2003 → 2011 → **2019 18-year follow-up** (94.8% implant + 99.2% prosthetic survival), Soto-Penaloza 2017 systematic review, Eliasson 2010, ITI 2019
- **Marcin's 5-step Entscheidungsalgorithmus** (CBCT → Lifestyle → Budget → Longevity → Preference)

#### L-6 #4 — Zahnimplantat-Komplikationen

**Migracja 140** (303 LOC):
- Slug: `zahnimplantat-komplikationen-periimplantitis-praevention-behandlung`
- 1892 słów DE, 20 cross-links, image `/kb-implant-structure.webp` (reuse L-6 #2)

**Content highlights**:
- **Frühe vs Späte Misserfolge** — definicja, ursachen, czasomierze
- **6 Haupt-Komplikationen**:
  - Periimplantäre Mukositis (reversible, 30-50% prevalence)
  - **Periimplantitis** (5-10% nach 10y per Mombelli 2012, bei Risikopatienten do 20%)
  - Schraubenlockerung (3-5% after 5y, häufiger u Single)
  - Implantatfraktur (<0.5%, extremely rare)
  - Progressiver Knochenverlust ohne Entzündung (3-7%)
  - Implantatverlust
- **Periimplantitis deep dive**:
  - Risikofaktoren z evidence (Heitz-Mayfield 2018, Strietzel 2007)
  - Symptoms (frühe vs späte warning signs)
...
- **Prävention**: Professional Nachsorge 3-6 mies + Sonicare + interdental + irrigator + Chlorhexidin + Rauchen reduzieren + Diabetes + Bruxguard
- **Red flags emergency**: starka schmerzen, mobilität, eitrige sekretion, plötzliche schwellung + fieber → 24/7 WhatsApp deutsch
- **Garantie reference** + 85-90% Reimplantation success rate

#### L-6 cluster cumulative — Faza M Tryb A early results

**4 articles × ~2000 słów = 7833 słów DE total + 74 cross-links**

L-6 cluster jest pierwszą falą **Faza M Tryb A** (AI-only rolling content engine 16-20 tygodni, target 60-80 artykułów). Cumulative dotychczas:

- **L-5 pillar**: 2050 słów (Zahnimplantate Hauptartikel) — verdict A
- **L-6 #1**: 1890 słów (Knochenaugmentation)
- **L-6 #2**: 2215 słów (Sofortbelastung)
- **L-6 #3**: 1836 słów (All-on-4 vs All-on-6)
- **L-6 #4**: 1892 słów (Komplikationen)
- **Total DE content batch**: **9883 słów** w 5 artykułach × ~3h AI każdy = ~15h AI total
...
| Lesson | Status |
|---|---|
| Implant pricing 6-8k PLN (no 4800 fake breakdown) | ✅ stosowane w L-6 #2-#4 |
| All-on-4 30-40k / All-on-6 45-55k PLN | ✅ stosowane w L-6 #2 + #3 |
| Zero embedded Reviews schema | ✅ all 4 articles |
| Standalone DE hreflang (own group_id) | ✅ all 4 articles |
| KB parser-compatible: lists, NO markdown tables | ✅ lesson z L-6 #2, stosowany w #3 + #4 |
| Clinic stats round-up + cross-link do /de live | ✅ all 4 articles |
| Cross-cluster reinforcement (L-5 pillar + inter-L-6 links) | ✅ wszystkie inter-cluster links present |
| Idempotent INSERT + kopie .txt | ✅ migracje 137-140 |

#### Faza L progress: 10/10 sesji done — FAZA L COMPLETE! 🎉

- L-1..L-4 ✅
- L-5 ✅ DE pillar (verdict A)
- L-9 ✅ FAQ DE/UA re-translate
- L-10 ✅ hreflang audit
- **L-6 cluster ✅ COMPLETE** (4/4 articles: Knochenaugmentation + Sofortbelastung + All-on-4/6 + Komplikationen)
- ⏳ L-7 EN batch (5 art) — następna duża faza
- ⏳ L-8 UA batch (3 art)

**Cała Faza L (Local + International Depth) ukończona dla DE locale.**

#### 🚨 Manual Marcin po deploy (CUMULATIVE L-6 cluster #1-#4)

1. **Wgrać 4 migracje** na OBU Supabase (wszystkie idempotent, można wgrać razem):
...
3. **Cumulative quick review ~25-30 min** (5-7 min per article):
   ...
   - **L-6 #1**: Erfolgsquote 95-98% Sinuslift / 90-95% GBR + Geistlich Bio-Oss/Bio-Gide/PRF zgodne z praktyką?
   - **L-6 #2**: ISQ ≥70 Osstell Beacon + Torque ≥35 Ncm + All-on-4 30-40k PLN — używasz Osstell? Cennik OK?
   - **L-6 #3**: Decision algorithm All-on-4 vs All-on-6 zgodne z Twoją praktyką? Hybrid +2 stosowane?
   - **L-6 #4**: Er:YAG laser periimplantitis decontamination — używasz w klinice? Behandlungsprotokoll OK?

4. **GSC re-submit sitemap** (~770 URLi po L-6 cluster) — auto-pickup ze articles table po wgrywce migracji
...
**Decyzja Marcina**:
- **(A) L-7 EN content batch start** — 5 artykułów EN (Knochenaufbau, Sofortbelastung, All-on-4, Komplikationen, plus 1 nowy temat — np. "Dental tourism Poland UK perspective"). ~15h AI rozłożone na 5 sesji.
- **(B) Pauza + Marcin medical review** L-5 + L-6 cluster przed L-7 start. Decyzja po review czy DE pattern OK przed inwestycją w EN.
- **(C) Schemat expansion w międzyczasie** — EventSchema dla LA&HA wykładów, BookSchema dla Czelej, ScholarlyArticle dla publikacji (niche E-E-A-T, 2h AI).
- **(D) GBP optimization audit** (opcja z planu PLAN_SEO_DALSZE_DZIAŁANIA_2026-05-25.md) — 1h AI + 2h Marcin.

---

### 2026-05-25 LATE+2 — 🇩🇪 L-6 #2: "Sofortbelastung beim Zahnimplantat" — drugi cluster article

**Drugi article L-6 cluster** po L-6 #1 Knochenaugmentation. 2215 słów DE + 19 cross-links (12 unique destinations) + migracja 138 idempotent INSERT. Cross-cluster reinforcement: L-5 pillar + L-6 #1 (Knochenaugmentation) cross-link.

#### Commit

- `0f6ab3a` — feat(seo): L-6 #2 — DE article "Sofortbelastung beim Zahnimplantat"
...
**Content** (2215 słów DE — solidnie więcej niż L-5/L-6 #1, 13 sekcji + 20 sub-sekcji):

1. **Einführung** — terminologia (Sofortbelastung vs Sofortversorgung — full occlusion vs occlusal entlastet)
2. **Voraussetzungen** (6 kryteriów evidence-based):
   - Primary stability ≥35 Ncm Insertion Torque
   - ISQ ≥70 (Osstell Beacon RFA — kluczowy paramter intraop)
   - Lekholm-Zarb bone density Type 1-3
   - Knochenvolumen 10mm vertikal / 6mm horizontal
   - Sterilność operacyjna
   - Patient compliance
3. **Risikofaktoren** (8 contraindications): heavy smoking >10/dzień, unkontrolowany DM, bisphosphonate IV, aktywna parodontosis, bruxismus bez splint, immunsuppresja, równoczesna augmentation, etc.
4. **2 Hauptszenarien**: immediate placement w extraction socket vs healed bone implantation
5. **All-on-4/6 deep dive**: anguliertes Setzen 15-30° distale Implantate (omijają Kieferhöhle → oft bez augmentation), cross-arch splinting, provisional w 24-48h, finale Brücke po 3-4 mies. **Kosten**: All-on-4 30-40k PLN, All-on-6 45-55k PLN
6. **Wissenschaftliche Evidenz** z literatura:
   - Esposito 2013 Cochrane Review (95-98% Einzelimplantat 5y)
   - Maló 2011 original All-on-4 study (96-98% 10y, n=245)
   - **Maló 2019 18-year follow-up** (94.8% implant + 99.2% prosthetic survival)
...
8. **Marcin's Sofortbelastung-Protokoll**:
   - Osstell ISQ obligatorisch (nie subjective Schätzungen)
   - Premium implants only — Straumann BLT, Nobel Replace CC, Astra Tech EV
   - PRF w Alveole (Marcin's LA&HA Forschungsschwerpunkt z Slovenien 2019/2023 + Polen Keynote 2022)
   - Conservative indication przy grenzfällen
   - Bruxguard obligatorisch dla immediate load patients
9. **Vergleich Sofort vs Konventionell** — list format (NIE table — KB parser doesn't support markdown tables, lesson z tej sesji)
10. **Kosten**: single 6-8k PLN (im Standardpaket), All-on-4 30-40k PLN, All-on-6 45-55k PLN, Vergleich Deutschland All-on-4 18-30k EUR — Ersparnis 50-70%
11. **Anreise**: dłuższy pierwszy pobyt 3-5 dni dla immediate load (operacja + provisional + kontrolla)
12. **FAQ 6 Q&A**: ból intensywność, fail handling (2-5% misserfolg ratio), żucie restrictions 6-8 weeks, visible lücke (NIE — provisional 24-48h), Rauchen 5-10/dzień scenario, All-on-4 bez augmentation
13. **Nächste Schritte + Kontakte**

**19 cross-links** (12 unique destinations):
- **L-5 pillar** (`/de/baza-wiedzy/zahnimplantate-in-polen-kosten-qualitaet-opole`) — cross-cluster reinforcement
- **L-6 #1 cross-cluster** (`/de/baza-wiedzy/knochenaugmentation-in-polen-methoden-kosten-erfolgsquote`) — first cluster-to-cluster link w Faza M!
...
**KB parser nie obsługuje markdown tables**. Dodałem porównanie Sofortbelastung vs Konventionell jako table `| col | col |` — przerobione na format list (`**Behandlungsdauer:** \n* Sofortbelastung — ...`). Pattern dla przyszłych articles: ZERO tables markdown, użyj headed list groups.
...
| Standalone DE hreflang | Own group_id, hreflang DE + x-default → DE |
| KB parser inline links | 19 cross-links across content |
| KB parser NIE tables | Vergleich section jako list format |
| Clinic stats round-up | "über 1.250 Implantate" + cross-link do /de live |
| Idempotent INSERT | WHERE NOT EXISTS guard + kopia .txt |

#### Build + verification

- `npm run build` clean (exit 0, no errors)
- 2215 słów DE + 19 cross-links + 13 h3 + 20 h4 + 246 bold markers + 0 markdown tables
- Content compliance grep: no 4800 PLN false breakdown, Straumann/Nobel/Astra/Osstell brand references present

#### 🚨 Manual Marcin po deploy

1. **Wgrać migrację 138** na OBU Supabase: `~/Desktop/migracje_supabase/migracja_138_l6_de_sofortbelastung_article.txt`. Idempotent. **Plus nadal wgrywka migracji 137** (L-6 #1) jeśli jeszcze nie zrobiona.
...
3. **Quick review ~5-10 min** (wzorzec L-5 ustalony):
   - Fluency DE
   - ISQ ≥70 + Torque ≥35 Ncm — OK z Twoją praktyką?
   - All-on-4 30-40k PLN + All-on-6 45-55k PLN — verify wobec aktualnego cennika kliniki
   - Osstell Beacon usage opis — czy używasz Osstell? Jeśli nie, alternative wording można zmienić w przyszłej revision
...
- L-5 ✅ DE pillar (verdict A)
- L-9 ✅ FAQ, L-10 ✅ hreflang audit
- L-6 #1 ✅ Knochenaugmentation
- **L-6 #2 ✅** Sofortbelastung (TEN sprint)
- ⏳ L-6 #3 All-on-4 vs All-on-6 (możliwy short / lub already covered in L-6 #2 deep dive)
- ⏳ L-6 #4 Zahnimplantat-Komplikationen
- ⏳ L-7 EN batch (5 art)
- ⏳ L-8 UA batch (3 art)
...
**L-6 #2 ma juz extensywne All-on-4/6 deep dive section** (Anguliertes Setzen 15-30°, Maló 2011/2019 evidence, cross-arch splinting, costs 30-55k PLN). Pytanie: czy L-6 #3 "All-on-4 vs All-on-6" wciąż potrzebny jako osobny article, czy zamiast tego napiszemy L-6 #3 "Zahnimplantat-Komplikationen" (z planu L-6 #4)? Wstępna decyzja Marcina na początku następnej sesji.

---

### 2026-05-25 LATE+1 — 🇩🇪 L-6 #1: "Knochenaugmentation in Polen" — DE article (Faza M Tryb A start)

**Pierwszy cluster article L-6 po Marcin verdict A na L-5 quality** (2026-05-25). Faza M Tryb A confirmed → rolling AI-only content engine started. 1890 słów DE + 14 cross-links + idempotent migracja 137.

#### Commit

- `6efc446` — feat(seo): L-6 #1 — DE article "Knochenaugmentation in Polen" (Faza M Tryb A confirmed)
...
**Content** (1890 słów DE, 11 sekcji + 15 sub-sekcji):
1. **Einführung** — kiedy Knochenaugmentation potrzebna (~30-40% przypadków implantacji, atrofia/periodontosis/anatomia)
...
3. **Premium-Materialien**: Bio-Oss (gold standard 30+ lat, histologie 40+ lat), Bio-Gide membrana, PRF (Marcin's specjalność LA&HA), Straumann MembraGel, Cerasorb synthetic alternative
4. **Kosten**: indywidualnie po konsultacji (lesson z mig 134 — no fake breakdown). Realistyczna spanne 1500-4500 PLN (350-1050 EUR) vs Deutschland 1500-5000 EUR. Ersparnis 40-60%
5. **Behandlungsablauf 5 Schritte**: CBCT planning → operacja 60-120 min → Heilphase 4-9 mies → Implantatsetzung → prothetische Versorgung
...
8. **Klinik stats (round-up + cross-link strategy)**: "über 1.250 Implantate, tausende Wurzelkanalbehandlungen, über 6.000 Patienten" + cross-link do `/de` homepage live TrustStats (clinic-wide live z Prodentis, jak Marcin decided 2026-05-25)
9. **Anreise**: 1-2 wizyty dla augmentacji + powrót po 4-9 mies dla implantu, A4 Dresden/Berlin/Wien times
10. **Kostenerstattung EU 2011/24**: krótka section + cross-link do /de/gwarancje (główne info tam)
11. **FAQ 6 Q&A**: czas, ból, alternatywy (kurze Implantate, All-on-4, brücke, denture), Bio-Oss safety (xenogen worry), wielokrotne augmentacje, fail handling
12. **Nächste Schritte**: Tel/WhatsApp +48 570 270 470 (DE), CTAs do rezerwacja/kontakt + 4 cross-links

**14 cross-links internal**:
- L-5 pillar (3×): `/de/baza-wiedzy/zahnimplantate-in-polen-kosten-qualitaet-opole`
- `/de/akredytacje`, `/de/cennik`, `/de/zespol/marcin-nowosielski`
- `/de/dla-pacjentow-przyjezdnych`
- `/de/gwarancje` (3×: Versicherung, EU-erstattung, Behandlungsgarantie)
- `/de/oferta/implantologia`, `/de/rezerwacja`, `/de/kontakt`
- `/de` (homepage live stats — Marcin's clinic-wide strategy)
- `/implanty-opole` (PL-only geo intencional, dla pacjentów lokalnych)

#### Lessons z poprzednich problemów applied

| Lesson | Action |
|---|---|
| Implant pricing fix (mig 134) — kein 4800 PLN | Augmentation 1500-4500 PLN realistic, no fake breakdown |
| GSC self-serving Reviews (mig 135) | Zero embedded Reviews schema, tylko Article + BreadcrumbList |
| Standalone DE hreflang pattern (L-5) | Own group_id, hreflang tylko DE + x-default → DE |
| KB parser refactor (L-5 renderInline) | Liberal użycie `[text](url)` inline links — 14 cross-links |
| K-4/K-5 typograficzne cudzysłowy issue | SQL `$body$` heredoc (nie JSON), bezpieczne dla cudzysłowów |
| Idempotent migracje (mig 133) | WHERE NOT EXISTS guard + kopia .txt |

#### Build + verification

- `npm run build` clean (exit 0, no errors)
- 1890 słów DE + 14 unique cross-links + 11 h3 + 15 h4 + 180 bold markers
- No live URL test (article czeka na Marcin wgrywkę migracji 137 na obu Supabase)

#### 🚨 Manual Marcin po deploy

1. **Wgrać migrację 137** na OBU Supabase (produkcja + demo): `~/Desktop/migracje_supabase/migracja_137_l6_de_knochenaugmentation_article.txt`. Idempotent.
2. Live URL po wgraniu: https://www.mikrostomart.pl/de/baza-wiedzy/knochenaugmentation-in-polen-methoden-kosten-erfolgsquote
3. **Quick review ~5-10 min** (nie 30-40 jak L-5 — wzorzec ustalony):
   - Fluency DE spot-check
   - Liczby Erfolgsquote (95-98% Sinuslift, 90-95% GBR) zgodne z Twoją kliniczną wiedzą
   - Material brands (Geistlich Bio-Oss, Bio-Gide, PRF) zgodne z tym co używasz
...
- L-9 ✅ FAQ
- L-10 ✅ hreflang audit
- **L-6 #1 ✅** Knochenaugmentation (TEN sprint)
- ⏳ L-6 #2 Sofortbelastung (next)
- ⏳ L-6 #3 All-on-4 vs All-on-6
- ⏳ L-6 #4 Zahnimplantat-Komplikationen
- ⏳ L-7 EN batch (5 art)
- ⏳ L-8 UA batch (3 art)
...
L-6 #2 "Sofortbelastung — wann ist sie möglich? Voraussetzungen und Risiken" (~3h AI, same pattern). Po L-6 full done → L-7 EN batch (5 art).

---

### 2026-05-25 LATE — 🌐 L-10 — Extended hreflang audit (PL-only + globally-noindex + robots/canonical checks)

**Premium SEO Plan / Faza L / Sesja 7/10** — domknięcie L-10 z planu: cross-linking foreign content + audit hreflang. Rozszerzony skrypt z J-2 (2026-05-12) o pełną walidację nowych stron z Fazy L/K + Batch SEO-2 oraz check meta robots noindex + canonical link consistency. Wynik: 168/168 OK, 0 broken.

#### Commit

- `32990c8` — feat(seo): L-10 — extended hreflang audit (PL-only + globally-noindex + robots/canonical checks)

#### Co dodane do `scripts/audit-hreflang.mjs`

**1. PUBLIC_PATHS rozszerzone z 30 → 42 paths** (parity z sitemap.ts):
...
- `isNoindex(robots)` helper — `\bnoindex\b` test (handles "noindex", "noindex,follow", "noindex, follow", "follow,noindex")

**3. Per-mode validation**:
- **Multi-locale paths**: każda locale variant musi być indexable + canonical=self
- **PL_ONLY_PATHS**: PL locale indexable + canonical=PL; foreign locales (en/de/ua) MUST be noindex (foreign canonical może być dowolny — zwykle locale-aware self)
- **GLOBALLY_NOINDEX_PATHS**: noindex w każdym locale (transactional landings bez organic intent)
...
- `/zadatek` (J-2: thin content + URL params, no organic intent w żadnym locale)

#### Workflow + odkrycia podczas audytu

**Iteracja 1** (przed dodaniem GLOBALLY_NOINDEX): 167/168 OK, 1 broken — false positive na `/zadatek` w PL locale (audit traktował jako PL_ONLY z PL indexable, podczas gdy `/zadatek` jest globally noindex per J-2).

**Iteracja 2** (po dodaniu GLOBALLY_NOINDEX_PATHS): **168/168 OK, 0 broken** ✓

**Stan hreflang ecosystem produkcji**: WSZYSTKIE 42 paths × 4 locale są poprawnie skonfigurowane. Każda strona ma:
- Pełen 5-element hreflang circle (pl + en + de + uk + x-default) — bez błędu `hreflang="ua"`
- Self-link w alternates
- Canonical = self (multi-locale) lub PL (PL-only)
- Spójny meta robots zgodny z trybem strony (indexable / PL-only foreign noindex / globally noindex)
...
Exit code 1 jeśli regression detected → ready jako GitHub Actions workflow lub manual pre-deploy gate.

#### Cross-link audit Footer + Navbar — verification

**Footer** (`src/components/Footer.tsx`): używa `Link` z `@/i18n/navigation` (auto locale prefix) dla wszystkich internal links. **Decyzja architektoniczna**: PL-only paths (`/implanty-opole`, `/sklep`, `/regulamin` itp.) z foreign locale dadzą `/en/implanty-opole` etc. — content się renderuje w EN (translacje obecne), tylko `<meta robots noindex>` dla Google. **To jest poprawne UX**: EN user widzi EN content (nie force PL routing), Google ignoruje noindex w hreflang chain. Brak zmian wymaganych.

**Navbar** (`src/components/Navbar.tsx`): top categories tylko (Oferta, Cennik, Metamorfozy, Narzędzia ▾, Dodatki ▾, Kontakt). Geo pages L-1/L-2 + dedicated zespol pages dostępne via Footer + cross-links w content (akredytacje + zespol z `/o-nas`). Brak refaktoru wymagany.

#### Co to daje

L-10 cleanup task z Premium SEO planu zamknięty:
- Polish foreign URL ecosystem przed eventual L-6/L-7/L-8 content batches
- CI gate ready (skrypt zwraca exit 1 na regression)
- Verification że wszystkie nowe geo + warranty + akredytacje + zespol pages mają poprawny hreflang chain + canonical + robots pattern
- Future-proof: każda nowa strona dodawana do sitemap musi też trafić do PUBLIC_PATHS w audit (catch regressji)
...
- L-3 ✅ phone helper + Dentist DACH areaServed
- L-4 ✅ `/gwarancje` warranty hub
- L-5 ✅ DE article test (awaiting Marcin medical review + GSC pickup)
- L-9 ✅ DE/UA FAQ re-translate
- **L-10 ✅** cross-linking + hreflang audit (TEN sprint)
- ⏳ L-6 DE content batch (po L-5 decision)
- ⏳ L-7 EN content batch
- ⏳ L-8 UA content batch

#### Pliki

- `scripts/audit-hreflang.mjs` [MOD] — +163/-18 LOC (extended walidacje, nowe categories, nowe report sections)
- `package.json` [MOD] — +1 LOC `audit:hreflang` npm script
- `public/sw.js` + `src/lib/generated-route-mtimes.ts` [MOD] — auto-regenerated by prebuild

#### Brak DB / env var

Kod-only zmiana. Vercel auto-deploy applies natychmiast. Audit script może być uruchamiany lokalnie (preview server :3789) lub przeciwko produkcji (`AUDIT_BASE_URL=https://www.mikrostomart.pl`).

#### Co dalej (recommended next)

Per plan `~/Desktop/bałagan/PLAN_SEO_DALSZE_DZIAŁANIA_2026-05-25.md`:
- **Opcja A (czeka)**: L-5 medical review Marcina (blokuje L-6/L-7/L-8)
- **Opcja B (nowe)**: GBP optimization audit + Marcin update GBP (zdjęcia, posty, FAQ) — 1h AI + 2h Marcin
- **Opcja C (strategic)**: Long-tail content opportunity audit (GSC Performance export → 10-15 article ideas dla Fazy M)
- **Opcja D (schema expansion)**: EventSchema dla LA&HA wykładów + BookSchema dla książki Czelej Marcina (niche E-E-A-T boost)
...
#### Commit
- `8469d88` — feat(seo): L-9 — DE/UA FAQ re-translate cat0-cat9 (43 Q × 2 locale, deeper contextualization)

#### Audit przed L-9

Per question audit `messages/{pl,en,de,ua}/pages.json` namespace `faq`:
- **PL** 9828 chars (100% baseline)
- **EN** 9367 chars (**95%** — OK, skip)
- **DE** 6755 chars (**69%** — 31% krócej)
- **UA** 5894 chars (**60%** — 40% krócej, najgorzej)
- **Cat10** "Dla pacjentów przyjezdnych" już DE/UA **92-105%** (native-written w H6 2026-05-10) → skip

**Odkrycie kontrastu**: Cat10 dowodzi że da się pisać foreign FAQ na poziomie PL. Mechanical PL→DE/UA translations w cat0..cat9 były 27-37% krótsze bo gubiły wszystkie nuanse (medical context, brand references, conversion-boosting content).

#### Co zrobione

**43 odpowiedzi DE + 43 UA = 86 transformations** w cat0..cat9. Każda zamieniona z mechanical PL→DE/UA na **deep contextualized native**. Po update DE/UA są teraz **113-194%** długości PL — over-shoot zatwierdzony bo value-add usprawiedliwia.

**Per-locale strategy**:
- **DE** native German tone:
  - Krankenkasse / BEMA/GOZ references gdzie payment Q
  - "Marcin und Elżbieta sprechen Deutsch" + WhatsApp 24/7 na deutsch
...
- **UA** native Ukrainian medical terminology:
  - україномовний WhatsApp 24/7
  - страхова компанія context dla insurance Q
  - Prices PLN + USD conversion
  - Brand references identical do DE (Straumann/Nobel/ZEISS są ukraińskie tak samo)
  - "Доктор Марцін Новосельський (M.Sc. RWTH Aachen, лектор LA&HA)" w cat1q4
...
- Verify per-key: length, ratio PL, ⚠️ marker dla outliers (>130%)
...
- Claude_Preview headless localhost:3002:
  - `/de/faq` → 200, FAQPage schema valid, ZEISS+BEMA+RWTH+PLN+Marcin keywords all present
  - `/ua/faq` → 200, FAQPage schema valid, all keywords present
- Zero React errors, zero console warnings

#### Spodziewany efekt po Vercel deploy

- **Foreign FAQ rich snippets quality boost** w SERP DE/UA — Google rewards comprehensive answers + brand entity references
- **Conversion uplift** z FAQ → /rezerwacja — more confidence-building content (specific prices, brand names, doctor credentials)
...
- **GSC pickup 2-4 tyg**: impressions na DACH queries "Wurzelbehandlung Polen", "Zahnimplantat Polen Garantie", "Veneers Polen Erstattung", oraz UA queries "стоматолог Польща страхування"

#### Pliki

- `messages/de/pages.json` [MOD] — 43 keys w `faq` namespace zaktualizowane
- `messages/ua/pages.json` [MOD] — 43 keys w `faq` namespace zaktualizowane
- `public/sw.js` + `src/lib/generated-route-mtimes.ts` [MOD] — auto-regenerated by prebuild

#### Faza L status: 6/10 sesji done

- L-1..L-4 ✅ (2026-05-22)
- L-5 ✅ DE article test (2026-05-22 LATE) — awaiting Marcin medical review + GSC pickup
- **L-9 ✅** foreign FAQ re-translate (TEN sprint)
- ⏳ L-6 DE content batch (po L-5 decision)
- ⏳ L-7 EN content batch
- ⏳ L-8 UA content batch
- ⏳ L-10 cross-linking + audit hreflang

**Brak migracji DB ani env var. Brak manual tasków Marcina** — content content update applies natychmiast po Vercel deploy.

---

### 2026-05-22 LATE — 🚨 L-5: AI-only DE article test "Zahnimplantate in Polen" + KB markdown parser inline links

**L-5 = decision point Fazy M Tryb A** (16-20 tygodni rolling content engine, AI-only experiment). 2000-słowowy artykuł DE jako foreign content quality benchmark. Decision po Marcin medical review + GSC pickup w 2-4 tyg.

#### Commit
- `859b965` — feat(seo): L-5 — DE article "Zahnimplantate in Polen" + KB parser inline links

#### Co zrobione

**Artykuł DE "Zahnimplantate in Polen 2026: Kosten, Qualität, Erfahrungen aus Opole"** (~2050 słów, 9 sekcji):
- Einführung — warum DE pacjenci wybierają Polskę
- Kosten 2026 — breakdown PL vs DE (minPrice 4800 PLN, breakdown po pozycjach)
- Qualität — ZEISS mikroskop + PRF + laser Er:YAG/Nd:YAG + Straumann/Nobel/Astra + akredytacje PTE/ESE/PTSL/LA&HA
- Über uns — Marcin RWTH M.Sc. + 1150 implantów + 4295 pacjentów + publikacje
- Behandlungsablauf — 5-step (Erstberatung → Implantation → Osseointegration → Abutment → Krone), 2-3 wizyty DE pacjenta
- Anreise z DACH — czasy z Dresden/Berlin/München/Wien, lotniska Wrocław/Kraków, A4
- Kostenerstattung EU 2011/24 — Kostenvoranschlag → Anmeldung → Behandlung → Erstattung
- FAQ 6 Q&A
- Nächste Schritte — kontakt + cross-links

**8 cross-linków internal**: `/de/cennik`, `/de/akredytacje`, `/de/zespol/marcin-nowosielski`, `/de/dla-pacjentow-przyjezdnych`, `/de/gwarancje`, `/de/kontakt`, `/de/oferta/implantologia`, `/implanty-opole`.

**Migracja 133** `133_l5_de_implants_article.sql` — idempotent INSERT z `WHERE NOT EXISTS` guard. Standalone DE (nowy `group_id`, brak PL parent). Hreflang tylko DE + x-default → DE. Image: `/kb-implant-procedure.webp` (existing asset). Published 2026-05-22.

**Markdown parser refactor** `src/app/[locale]/baza-wiedzy/[slug]/page.tsx` (+30/-13 LOC): nowy helper `renderInline(text, keyPrefix)` obsługuje **bold** + `[text](url)` inline links. Wcześniej parser wspierał tylko `**bold**` w paragraphs/list items, brak anchor links → cross-linki w KB articles były niemożliwe. External URL detection (regex `^https?://`) → automatic `rel="noopener noreferrer" + target="_blank"`. Internal URLs (relative) → plain href. **Korzyść**: każdy przyszły KB article może mieć klikalne linki.

**Regression**: istniejący DE article `die-magie-des-lachelns-...` renderuje się 200 OK po refactorze (Article schema + BreadcrumbList present, zero console errors, **bold** nadal działa). Verified Claude_Preview headless.

#### Verification

- `npx tsc --noEmit` clean
- `npm run build` clean
- Claude_Preview headless: `/de/baza-wiedzy/die-magie-des-lachelns-...` → 200, Article schema, BreadcrumbList, strong tags, zero console errors

#### 🚨 Manual Marcin po deploy (krytyczne — bez tego artykuł nie jest live)

1. **Wgrać migrację 133** na OBU Supabase (produkcja `keucogopujdolzmfajjv` + demo `mhosfncgasjfruiohlfo`): `~/Desktop/migracje_supabase/migracja_133_l5_de_implants_article.txt`. Idempotent (`WHERE NOT EXISTS` guard) — bezpieczne re-run.

2. Po wgraniu live URL: `https://www.mikrostomart.pl/de/baza-wiedzy/zahnimplantate-in-polen-kosten-qualitaet-opole`

3. **Medical review** (~30-40 min): fluency DE, content accuracy (1150 implantów, 4800 PLN, M.Sc. RWTH OK), medical terminology (Osseointegration, Vollkeramik, BEMA/GOZ), tone premium ale not pompous.

4. **Google Rich Results Test**: sprawdzić Article schema + BreadcrumbList eligibility.

5. **GSC re-submit sitemap** (~750 URLi po dodaniu artykułu — sitemap.ts fetch z articles table, automatic pickup).

#### Decision point Fazy M (po 2-4 tyg)

- **Quality OK + GSC impressions** → Tryb A confirmed, kontynuujemy:
  - **L-6** DE content batch (4 dodatkowe artykuły × ~2000 słów each)
  - **L-7** EN content batch (5 artykułów "Dental tourism Poland", "Why Opole", etc.)
  - **L-8** UA content batch (3 artykuły, niższy priorytet)
  - **Faza M** AI-only rolling 16-20 tyg, 60-80 artykułów total
- **Quality słaba** → renegocjacja: zewnętrzny native speaker DE (~150-300 PLN/artykuł). Albo Tryb B hybrid (AI draft + human native edit).

#### Pliki

- `src/app/[locale]/baza-wiedzy/[slug]/page.tsx` [MOD] — `renderInline` helper, list items + paragraphs używają go zamiast inline split
- `supabase_migrations/133_l5_de_implants_article.sql` [NEW] — 206 LOC INSERT idempotent
- `~/Desktop/migracje_supabase/migracja_133_l5_de_implants_article.txt` [NEW] — kopia dla Marcina
- `~/.claude/launch.json` [MOD] — dodany `mikrostomart-dev` config (port 3002) dla szybszych preview tests
- `public/sw.js` + `src/lib/generated-route-mtimes.ts` [MOD] — auto-regenerated by prebuild
...
- L-3 ✅ phone helper + Dentist DACH areaServed
- L-4 ✅ /gwarancje warranty hub
- **L-5 ✅ DE article test** (TEN sprint, awaiting Marcin medical review)
- ⏳ L-6 DE content batch (po decision)
- ⏳ L-7 EN content batch
- ⏳ L-8 UA content batch
- ⏳ L-9 foreign FAQ re-translate
- ⏳ L-10 cross-linking + audit hreflang
...
> ℹ️ **Inwentarz referencyjny** (co istnieje / co znane-niedokończone = dług techniczny). Pozycje `[ ]` poniżej to **obserwacje / dług techniczny, NIE aktywna kolejka zadań** — od 2026-06-08 carte blanche (reset backlogu, patrz linia 3).
