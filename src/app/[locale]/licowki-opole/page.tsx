...
/**
 * /licowki-opole — Faza 2D premium local geo page (2026-06-08).
 *
 * Cel: ranking na geo queries "licówki Opole", "licówki porcelanowe Opole",
 * "projektowanie uśmiechu Opole", "Hollywood smile Opole". PL-only indexable,
 * foreign noindex (layout). Komplementarny do /oferta/stomatologia-estetyczna
 * (deep content) i /metamorfozy. Wzorzec implanty-opole / all-on-4-opole.
 */
export default async function LicowkiOpolePage({ params }: { params: Promise<{ locale: string }> }) {
        ...
        <main>
            ...
            <section style={{ padding: 'var(--spacing-xl) 0', background: 'linear-gradient(180deg, var(--color-background) 0%, var(--color-surface) 100%)' }}>
                <div className="container" style={{ maxWidth: '900px', textAlign: 'center' }}>
                    <RevealOnScroll priority>
                        <p style={{ color: 'var(--color-primary)', textTransform: 'uppercase', letterSpacing: '0.2em', fontSize: '0.85rem', marginBottom: 'var(--spacing-sm)' }}>
                            {t('heroTagline')}
                        </p>
                        <h1 style={{ fontSize: 'clamp(2.2rem, 5vw, 3.5rem)', marginBottom: 'var(--spacing-sm)', lineHeight: 1.1, fontWeight: 400 }}>
                            {t('heroTitle')} <span style={{ color: 'var(--color-primary)', fontStyle: 'italic' }}>{t('heroAccent')}</span>
                        </h1>
                        <p style={{ fontSize: '1.15rem', color: 'var(--color-text-muted)', marginBottom: 'var(--spacing-md)', lineHeight: 1.6, maxWidth: '720px', marginLeft: 'auto', marginRight: 'auto' }}>
                            {t('heroLead')}
                        </p>
                        <div style={{ display: 'flex', gap: 'var(--spacing-sm)', justifyContent: 'center', flexWrap: 'wrap' }}>
                            <Link href="/rezerwacja" style={{ display: 'inline-block', background: 'var(--color-primary)', color: 'var(--color-background)', padding: '1rem 2.5rem', fontSize: '1.05rem', fontWeight: 600, textDecoration: 'none', borderRadius: '2px', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                                {t('heroCta')}
                            </Link>
                            <Link href="/oferta/stomatologia-estetyczna" style={{ display: 'inline-block', background: 'transparent', color: 'var(--color-text-main)', padding: '1rem 2.5rem', fontSize: '1.05rem', fontWeight: 600, textDecoration: 'none', borderRadius: '2px', letterSpacing: '0.05em', textTransform: 'uppercase', border: '1px solid var(--color-surface-hover)' }}>
                                {t('heroCtaSecondary')}
                            </Link>
                        </div>
                    </RevealOnScroll>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'var(--spacing-md)' }}>
                        {[1, 2, 3].map((n) => (
                            <RevealOnScroll key={n}>
                                <article style={{ background: 'var(--color-surface)', border: '1px solid var(--color-surface-hover)', borderRadius: '4px', padding: 'var(--spacing-md) var(--spacing-md)', height: '100%' }}>
            ...
            <section style={{ padding: 'var(--spacing-xl) 0', background: 'var(--color-surface)' }}>
                <div className="container" style={{ maxWidth: '820px' }}>
                    ...
                    <ol style={{ listStyle: 'none', counterReset: 'step', padding: 0 }}>
                        {[1, 2, 3, 4, 5].map((n) => (
                            <li key={n} style={{ background: 'var(--color-background)', border: '1px solid var(--color-surface-hover)', borderRadius: '4px', padding: 'var(--spacing-md)', marginBottom: 'var(--spacing-sm)', counterIncrement: 'step', position: 'relative', paddingLeft: 'calc(var(--spacing-md) * 2 + 1.5rem)' }}>
                                <span style={{ position: 'absolute', left: 'var(--spacing-md)', top: 'var(--spacing-md)', background: 'var(--color-primary)', color: 'var(--color-background)', width: '2rem', height: '2rem', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '1rem' }} aria-hidden="true">{n}</span>
                    ...
                    <p style={{ textAlign: 'center', marginTop: 'var(--spacing-md)', color: 'var(--color-text-muted)', fontSize: '0.95rem' }}>
                        {t('procedureDeepLink')}{' '}
                        <Link href="/oferta/stomatologia-estetyczna" style={{ color: 'var(--color-primary)', fontWeight: 600 }}>{t('procedureDeepLinkLabel')}</Link>
                    </p>
                </div>
            </section>
            ...
            <section style={{ padding: 'var(--spacing-xl) 0' }}>
                <div className="container" style={{ maxWidth: '1100px' }}>
                    ...
                    <div style={{ marginTop: 'var(--spacing-lg)' }}>
                        <LazyMapEmbed src={brand.mapEmbedUrl} placeUrl={brand.googlePlaceId ? `https://www.google.com/maps/place/?q=place_id:${brand.googlePlaceId}` : brand.mapEmbedUrl} title={t('locationHeading')} />
                    </div>
                </div>
            </section>
            ...
            <section style={{ padding: 'var(--spacing-xl) 0', background: 'var(--color-surface)' }}>
                <div className="container" style={{ maxWidth: '820px' }}>
                    ...
                    {[1, 2, 3, 4, 5, 6].map((n) => (
                        <details key={n} style={{ background: 'var(--color-background)', border: '1px solid var(--color-surface-hover)', borderRadius: '4px', marginBottom: 'var(--spacing-sm)', padding: 'var(--spacing-sm) var(--spacing-md)' }}>
            ...
            {/* CTA */}
            <section style={{ background: 'linear-gradient(135deg, var(--color-surface) 0%, var(--color-background) 100%)', padding: 'var(--spacing-xl) 0', borderTop: '1px solid var(--color-surface-hover)' }}>
                <div className="container" style={{ maxWidth: '720px', textAlign: 'center' }}>
                    <RevealOnScroll>
                        ...
                        <Link href="/rezerwacja" style={{ display: 'inline-block', background: 'var(--color-primary)', color: 'var(--color-background)', padding: '1rem 2.5rem', fontSize: '1.05rem', fontWeight: 600, textDecoration: 'none', borderRadius: '2px', letterSpacing: '0.05em', textTransform: 'uppercase' }}>{t('ctaButton')}</Link>
