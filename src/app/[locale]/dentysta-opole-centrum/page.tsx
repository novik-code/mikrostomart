 ...
 *
 * Generic queries: "dentysta Opole", "dentysta Opole centrum", "stomatolog Opole".
 * PL-only — foreign noindex w layout.tsx. Pełen zakres usług w jednym landing
 * z 8 service items (linki do /oferta/* i innych geo pages).
 *
 * Struktura: Hero + 3 USPs + 4-step "jak rozpocząć" + 8 services list +
 * Lokalizacja + Zespół + Reviews + 6 FAQ + CTA.
 */
export default async function DentystaOpoleCentrumPage({ params }: { params: Promise<{ locale: string }> }) {
                ...
                <div className="container" style={{ maxWidth: '900px', textAlign: 'center' }}>
                    <RevealOnScroll priority>
                        ...
                        <div style={{ display: 'flex', gap: 'var(--spacing-sm)', justifyContent: 'center', flexWrap: 'wrap' }}>
                            <Link href="/rezerwacja" style={{ display: 'inline-block', background: 'var(--color-primary)', color: 'var(--color-background)', padding: '1rem 2.5rem', fontSize: '1.05rem', fontWeight: 600, textDecoration: 'none', borderRadius: '2px', letterSpacing: '0.05em', textTransform: 'uppercase' }}>{t('heroCta')}</Link>
                            <Link href="/cennik" style={{ display: 'inline-block', background: 'transparent', color: 'var(--color-text-main)', padding: '1rem 2.5rem', fontSize: '1.05rem', fontWeight: 600, textDecoration: 'none', borderRadius: '2px', letterSpacing: '0.05em', textTransform: 'uppercase', border: '1px solid var(--color-surface-hover)' }}>{t('heroCtaSecondary')}</Link>
                        </div>
                    </RevealOnScroll>
                </div>
            </section>
            ...
            <section style={{ padding: 'var(--spacing-xl) 0', background: 'var(--color-surface)' }}>
                <div className="container" style={{ maxWidth: '820px' }}>
                    ...
                    <p style={{ textAlign: 'center', marginTop: 'var(--spacing-md)', color: 'var(--color-text-muted)', fontSize: '0.95rem' }}>
                        {t('procedureDeepLink')}{' '}
                        <Link href="/oferta" style={{ color: 'var(--color-primary)', fontWeight: 600 }}>{t('procedureDeepLinkLabel')}</Link>
                    </p>
                </div>
            </section>

            {/* Pełen zakres usług — 8 services */}
            <section style={{ padding: 'var(--spacing-xl) 0' }}>
                <div className="container" style={{ maxWidth: '900px' }}>
                    <RevealOnScroll>
                        <h2 style={{ fontSize: 'clamp(1.6rem, 3.5vw, 2.2rem)', textAlign: 'center', color: 'var(--color-primary)', marginBottom: 'var(--spacing-sm)' }}>{t('servicesHeading')}</h2>
                        <p style={{ color: 'var(--color-text-muted)', textAlign: 'center', fontSize: '1rem', marginBottom: 'var(--spacing-lg)' }}>{t('servicesIntro')}</p>
                    </RevealOnScroll>
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 'var(--spacing-sm)' }}>
                        {[1, 2, 3, 4, 5, 6, 7, 8].map((n) => (
                            <li key={n} style={{
                                background: 'var(--color-surface)', border: '1px solid var(--color-surface-hover)',
                                borderRadius: '4px', padding: 'var(--spacing-sm) var(--spacing-md)',
                    ...
                    <p style={{ textAlign: 'center', marginTop: 'var(--spacing-md)', fontSize: '0.95rem' }}>
                        <Link href="/cennik" style={{ color: 'var(--color-primary)', fontWeight: 600 }}>Zobacz pełen cennik →</Link>
                    </p>
                </div>
            </section>
            ...
            <section style={{ padding: 'var(--spacing-xl) 0', background: 'var(--color-surface)' }}>
                <div className="container" style={{ maxWidth: '1100px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 'var(--spacing-md)' }}>
                        {[1, 2, 3, 4].map((n) => (
                            <RevealOnScroll key={n}>
                                <article style={{ background: 'var(--color-background)', border: '1px solid var(--color-surface-hover)', borderRadius: '4px', padding: 'var(--spacing-md)', height: '100%' }}>
                ...
                <div className="container" style={{ maxWidth: '820px' }}>
                    <RevealOnScroll>
                        ...
                        <div style={{ textAlign: 'center' }}>
                            <Link href="/o-nas" style={{ color: 'var(--color-primary)', fontWeight: 600, textDecoration: 'none', fontSize: '1rem' }}>{t('doctorCta')}</Link>
                        </div>
                    </RevealOnScroll>
                </div>
            </section>
                ...
                <div className="container" style={{ maxWidth: '820px' }}>
                    ...
                    {[1, 2, 3, 4, 5, 6].map((n) => (
                        <details key={n} style={{
                            ...
                            <p style={{ color: 'var(--color-text-muted)', fontSize: '0.95rem', lineHeight: 1.6, marginTop: 'var(--spacing-sm)', paddingTop: 'var(--spacing-sm)', borderTop: '1px solid var(--color-surface-hover)' }}>{t(`faqA${n}`, brandI18nParams())}</p>
                        </details>
                    ))}
                </div>
            </section>
