...
/**
 * Sekcja zapisana w `site_settings.value` (JSON zapisywany przez panel admina).
 * Kształt taki sam jak `PageSection`, ale `type` jest zwykłym stringiem —
 * w DB mogą siedzieć typy spoza aktualnego union `SectionType`.
 */
...
export async function GET() {
    try {
        ...
        const dbSections = data.value as StoredSection[];
        const dbByType = new Map(dbSections.map((s): [string, StoredSection] => [s.type, s]));
        ...
        const merged: StoredSection[] = templateSections.map((t, idx) => {
        ...
        const extraDbSections = dbSections.filter((s) =>
            !templateSections.some(t => t.type === s.type)
        );
        extraDbSections.forEach((s, idx) => {
