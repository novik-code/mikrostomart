// GET /api/employee/time-tracking-self/report?month=YYYY-MM&format=pdf|csv
// Pracownik pobiera własny raport miesięczny.
...
export async function GET(request: NextRequest) {
    ...
    if (!month || !/^\d{4}-\d{2}$/.test(month)) {
        return NextResponse.json({ error: 'Brak month (YYYY-MM)' }, { status: 400 });
    }
    ...
    // Pracownik NIE widzi własnej hourly_rate w raporcie (decyzja: informacja zarządcza)
    ...
    const totals = {
        days: shifts.length,
        worked_minutes: 0, planned_minutes: 0,
        late_minutes: 0, early_leave_minutes: 0,
        overtime_total_minutes: 0, overtime_justified_minutes: 0, overtime_unjustified_minutes: 0,
        absence_days: 0, days_with_anomalies: 0,
    };
    ...
    const reportData: ReportData = {
        employee: { id: employee.id, name: employee.name, position: employee.position, contractType },
        month, workingDays, dailyHours, normaMinutes, hourlyRate,
        shifts, totals,
    };

    const safeName = employee.name.normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/\s+/g, '_');
