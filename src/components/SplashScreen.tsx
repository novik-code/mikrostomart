...
export default function SplashScreen({ children }: { children: React.ReactNode }) {
    ...
    // Stable JSX tree — NEVER changes structure to avoid React remount flicker
    return (
        <>
            {/* Page content — ALWAYS in this exact wrapper, opacity controlled */}
            <div style={{
            ...
            {/* Overlay — ALWAYS rendered, hidden via CSS not conditional JSX */}
            <div
                style={{
                    position: 'fixed',
                    inset: 0,
                    ...
                    opacity: showOverlay ? (isLogoFlying ? 0 : 1) : 0,
                    visibility: showOverlay ? 'visible' : 'hidden',
                    transition: 'opacity 1.2s ease-in-out, visibility 1.2s ease-in-out',
                    pointerEvents: showOverlay && !isLogoFlying ? 'auto' : 'none',
                }}
                onClick={showOverlay ? handleSkip : undefined}
            >
                ...
                {shouldShow && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: showOverlay && !isLogoFlying ? 0.3 : 0 }}
